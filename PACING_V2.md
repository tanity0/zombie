...
## 3. AIディレクター(旧・反応型システム)の再統合(社長指示・v0.26.10〜)
...
シグナル計算自体は現状ですでにデフォルト有効(実装チャット調査済み・v0.26.10チャット参照)。
今回の指示で変えるのは以下の2点:
- **リラックスモード等(`relaxSpawnAdjust`/`buildupSpawnAdjust`、macro='relax'/'buildup'に応じた
  湧きテンポ/上限緩和)をデフォルトON化。** 現状は`?directorApply=relax|buildup|all`を付けない限り
  無効(`DIRECTOR_APPLY_RELAX`/`DIRECTOR_APPLY_BUILDUP`がデフォルトfalse)。これをデフォルトtrue
  (常時有効)へ変更する。**ゲームプレイ中の可視表示(HUD/バナー等)は追加しない**(社長指示
  「ゲームプレイ中は表示しないで」)。挙動のみに効かせる=現状もHUD無し・変更なし。
- **リザルト画面の緊張曲線・難易度スコア(`DirectorResult`)をデフォルト表示化。** 現状は
  `?director=1`を付けないと出ない。これをゲーム終了後のリザルト画面で常時表示にする
  (プレイ中には出さない・現状どおりリザルト限定の表示)。
- ★未決事項候補(実装前に確認が必要):
  1. `relaxSpawnAdjust`/`buildupSpawnAdjust`(反応型macro)は、R1のgatePressure(台本ベースの
     連続テンポ)・R4のreliefProgram(緩の演目選択)と**別レバーとして同時に効かせてよいか**、
     それとも競合(二重に緩める/二重に強める)を避けるため、どちらかに絞るべきか。
  2. デフォルトON化にあたり、`relaxSpawnAdjust`/`buildupSpawnAdjust`の中身(閾値・倍率)は
     現状のまま流用でよいか(社長裁定済み扱いにしてよいか)、実機で見てから微調整するか。
  内容が出揃い次第、実装チャットで反映する。
