...
(R1)・浅いエリア代替表現(R4-C)とは別枠のアンビエント層**(フェーズ種別に依存せず、場所=エリアが
危険度を決める。「エリア表=何を出すか」「ディレクター(§3)=いつ出すか」の分業)。
...
- 安全弁: リフラクトリ15秒**維持**/同時数キャップ(上記)/総数enemyCap内/§3ディレクター連携
  (RELAX中・intensity≥0.75・pity中は発火しない)。boardDebtはこの経路では参照しない
  (キャップ+ディレクター連携が同役を担う)。
...
- エリア台本(§2)との連携(実装は§2の数値確定後): §2の「たまに」発火条件にディレクターのシグナル
  を使う(intensity高/RELAX中/pity発動中は発火させない)。「エリア表=何を出すか」「ディレクター=
  いつ出すか」の分業で、憲法廃止後の安全弁を務める。
...
| R7 | エリア台本(§2)+ディレクター連携ゲート(§3の「たまに」発火条件) | **完了(v0.26.13)** |
