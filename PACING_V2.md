...
4. **安全弁は残す**(pressure/boardDebt/pity/同時数キャップ)。ただし役割は「台本を選ぶ」ではなく
   「出し方を安全にする」。**台本は見せる。危険度は状況で抑える。**
5. **浅いエリアでも“違い”と問題児を見せる**。初心者ゾーンの問題児禁止(旧憲法4条)は撤回し、
   エリア1でもパンプキン等を止めない。危険度は同時数キャップ・ディレクター連携・pityで抑える。
...
## 1. 不変条件(全バッチ共通)
- **【v0.26.8時点で改定・社長指示】憲法(`constitution.test.ts`)は廃止。** 第1〜5条を含め
  「全条維持」の縛りは無くなった。初心者ゾーン(エリア0-1)不可侵(旧第4条)も対象。
  以降の設計・実装は上記§2「エリア台本」を含む本書の現行記述に従う。
  【v0.26.13対応済み】`constitution.test.ts`は「旧・憲法テスト」として存置(ヘッダで拘束力廃止を
  明記)。中身は現存する旧機構(countCap/`?v2=0`復帰用のゾーン天井/床許可リスト等)の回帰ガードとして
  残すが、本書の裁定があればいつでも書き換え・削除してよい(「全条維持」の縛りではない)。
...
## 2. エリア台本【R7・v0.26.13実装済み】
憲法廃止を受けて、エリア(距離帯)ごとに問題児の「たまに追加」を直接指定する。**関所の台本ローテ
(R1)・浅いエリア代替表現(R4-C)とは別枠のエリア圧レイヤー**(場所=エリアが危険度を決める。
ただし発火は実際のgateコマ中だけに限定し、導入・緩・ボスの呼吸は荒らさない。「エリア表=何を
出すか」「ディレクター(§3)=いつ出すか」の分業)。

| エリア | 内容(叩き台) |
|---|---|
| エリア1 | たまにパンプキン1体追加/たまに犬1体追加/たまに弾1-2体追加 |
| エリア2 | たまにパンプキン1体+弾1追加/たまに犬1体+弾1追加/たまに弾2体追加 |
| エリア3 | たまにパンプキン1体+弾2追加/たまに犬1体+弾2追加/たまに弾3体追加 |
| エリア4 | 塩梅任せ→実装: エリア3の3択+パンプキン1+犬1の混成ペアを追加した4択 |

- 上限(全エリア共通): パンプキン最大2/犬最大2/弾(plant)最大3の範囲に収める。
- 別枠(問題児とは別トラック): 叫び(screamer)最大1・ゴースト(ghost)最大2。
  3分経過で解禁、**ただしエリア3以降は開始直後から出現しうる**。

**「たまに」の較正(v0.26.13・社長指示「重みづけの概念を見ろ」に基づく実装確定・全て叩き台)**:
- ロール粒度15秒(問題児リフラクトリと同じ=倒した直後に次が来ない)。1ロールごとにエリア別確率で
  発火し、当該エリアの3(エリア4は4)択から一様に1セット選ぶ。
- 発火確率はCOLOR_RATE_BY_AREA(レア出現率)と同じ「エリア別確率配列」の形:
  **[エリア0=0, 0.20, 0.25, 0.30, 0.35]**(期待値: エリア1≈0.8セット/分〜エリア4≈1.4セット/分)。
  エリア0(軍備配置)は出さない。別枠(叫び/ゴースト)は**0.10**/ロールで控えめ。
- AREA_WEIGHTとの関係(v0.26.17社長指示): 26系では`AREA_WEIGHT=0`を「出現不可」として扱わない。
  エリア1でもパンプキン/犬などを通常抽選候補にできる。`?v2=0`の旧復帰経路だけ従来の
  AREA_WEIGHT制約を残す。
- 安全弁: リフラクトリ15秒**維持**/同時数キャップ(上記)/総数enemyCap内/§3ディレクター連携
  (gateコマ中のみ・RELAX中・intensity≥0.75・pity中は発火しない)。boardDebtはこの経路では参照しない
  (キャップ+ディレクター連携が同役を担う)。
- 実装: `src/utils/areaScript.ts`(純関数)+useGameLoop配線。**`?areascript=0`で無効化**。
- 旧・憲法4条(初心者ゾーン不可侵)との関係: 廃止。26系ではgatePressure側のゾーン天井
  (`ceilingForZone`)も実効天井に使わない。`ceilingForZone`は`?v2=0`の旧復帰経路用にだけ残す。

## 3. AIディレクター(旧・反応型システム)の再統合【裁定済みv0.26.12「推薦で」・実装済み】
エリア台本(§2)と合わせて、現行路線(R1〜R4)+旧AIディレクター(`src/utils/aiDirector.ts`)+
DirectorRank(難易度⑤・スコア付け)の「使える部分」を流用する。DirectorRankとgatePressureの
シグナル計算自体は現状ですでにデフォルト有効。**社長裁定(v0.26.12「推薦で」)により以下で確定・実装済み**:
- **リラックス(緩める側=`relaxSpawnAdjust`)のみデフォルトON。** `?directorApply=0`で無効化、
  `?directorApply=buildup`は従来どおり「buildupのみ」の意味を維持。**ゲームプレイ中の可視表示は
  追加しない**(社長指示「ゲームプレイ中は表示しないで」)=挙動のみ。
  湧き間隔のブレーキ(RELAX中×1.35)は、既存ブレーキ(boardDebt/パンプキン2体緩和)と**max合成**
  (GAME_AUDIT #4「ブレーキは二重掛けしない」の流儀)。閾値・倍率は現状値を流用(実機調整前提)。
- **buildup(強める側=`buildupSpawnAdjust`)はデフォルトOFF(opt-in)のまま。** Rank escBoost・
  退屈上振れ・gatePressureとアクセルが3重に重複するため(推薦どおり)。入れる場合は既存3レバー
  との優先関係を決めてから。
- **リザルト画面の緊張曲線・難易度スコア(`DirectorResult`)をデフォルト表示化。** `?director=0`で
  非表示。プレイ中には出さない(リザルト限定=従来の表示範囲のまま)。
- エリア台本(§2)との連携(実装は§2の数値確定後): §2の「たまに」発火条件にディレクターのシグナル
  と実フェーズを使う(gateコマ中のみ・intensity高/RELAX中/pity発動中は発火させない)。「エリア表=
  何を出すか」「ディレクター=いつ出すか」の分業で、憲法廃止後の安全弁を務める。

## 4. 台本選択の追加ルール【社長指示・v0.26.12実装済み】
R1の選択ロジック(`selectGateProgram`)への追加2ルール:
1. **非類似優先(「台本の選び方はできるだけ似ていないものを次に持ってくること」)**:
   各台本にテーマタグを持たせ、直前台本との類似度(共有タグ数)が最小の候補だけに絞ってから
   一様に選ぶ。適用順は 時間解禁→イベント選出ルール→直前除外→未見優先→**非類似優先**→一様選択
   (未見優先の後=未見の中で最も似ていないものを選ぶ)。
   タグ表(叩き台・実機調整前提):
   | 台本 | タグ |
   |---|---|
   | 数 | swarm(物量) |
   | 射線 | ranged(弾) |
   | 判断 | featured, ranged |
   | 三択 | featured, ranged, combo |
   | 不意打ち | featured, ranged, combo, surprise |
   | 襲撃 | event, swarm |
   | スパイク | event, featured |
   (例: 三択の後は{数, 襲撃}だけが候補になる=全部盛りの直後は毛色の違う物量系へ離れる)
2. **最高段に収束させない(「ランク7でも、ランク6の台本などを混ぜつつバリエーション」)**:
   選択はrankもmaxRungも一切参照しない(R1の既存原則を明文化)+直前禁止+非類似優先により、
   不意打ち(最高段rung7)解禁後も下段の台本が構造的に出続ける。`gateRotation.test.ts`(vi)で
   「7:00以降の関所の過半は不意打ち以外」を全数検証して機械化済み。
- `?v2=0`(旧rank寄せ選択)には影響しない(selectGateProgramLegacyは不変)。
...
- **破ってはならない**: 台本maxRung由来のpressure天井/同時数キャップ/リフラクトリ15秒。
  キャップ満杯なら空き待ちで投入(関所内で消化)。ゾーン天井は26系では使わない。
...
- guarantee: ユニットテストで「15秒経過・未出現→投入指示が立つ」「`?v2=0`旧経路のゾーン0-1では立たない」
  「キャップ満杯では待つ」を検証。
...
- PHASESの`maxRung`列は**撤去**(台本選択に使わなくなるため)。pressure天井=台本自身の
  maxRung由来のみになる(26系ではゾーン天井を掛けない)。`ceilingForZone`は`?v2=0`旧復帰経路用に残す。
...
### R4-C. 浅いエリアのテーマ表現(P2対応)【v0.26.6 定量化 / v0.26.17補正】
初心者ゾーン(エリア0-1)に居る関所では、チャフの湧き方を追加で変え、**代替ではなく追加表現**
としてテーマを見せる。v0.26.17以降、問題児自体は浅いエリアでも止めない。

**共通ルール(全パターン)**
- 発動条件: gateコマ中 ∧ **コマ開始時点**のプレイヤーが初心者ゾーン(エリア0-1) ∧ イベント関所ではない。
  コマ開始時に1回判定して固定(コマ途中のゾーン移動では切り替えない)。深いエリアでは発動しない
  (そこはR1-Dの主題保証が担う)。
...
| 襲撃 | ring | v0.26.17以降は本来イベントを止めないため、浅エリア代替から除外 |
| 三択 | waves | +5s/+25s/+45sの3波。各波**6体**を1方向±30°の扇状から。方向は波ごとに**120°ずつ回転**(初期角ランダム) |
| スパイク | burst | v0.26.17以降は本来イベントを止めないため、浅エリア代替から除外 |
...
- 初心者ゾーン滞在プレイでも関所ごとに湧き方と問題児の違いが目視で分かる(実機確認項目・統合テスト時)。
...
| R4 | 緩整理+浅エリア代替表現 | **完了(v0.26.8)。統合テスト待ち** |
| R5 | ゴールド経済接続 | **社長採否待ち・着手禁止** |
| R6 | ディレクター再統合(§3: relaxデフォON/リザルト常時表示)+台本選択追加ルール(§4: 非類似優先/最高段非収束) | **完了(v0.26.12)** |
| R7 | エリア台本(§2)+ディレクター連携ゲート(§3の「たまに」発火条件) | **完了(v0.26.13)。gate中限定へ補正(v0.26.16)。敵エリア制約撤廃(v0.26.17)** |

## 統合テスト待ち(社長指示・v0.26.6のテスト方針どおり)
R1〜R4を実装完了。この線は毎バッチの実機確認を行わず、R1〜R4を一気に実装してから統合テスト1回で
判定する方針(v0.26.6・PACING_V2.md運用節)。次のアクションは社長の統合テスト
(`?v2=1`(既定)/`?shallow=0`/`?zoomlock=1`での確認を推奨)。フィードバックがあれば
本書★未決事項または実装チャットへの直接指示で反映する。
...
### （経緯）R4-Cの実装範囲についての質問 → v0.26.6で定量化・全6種実装の指示を受けて解消
- Sonnetは当初、射線/判断/三択の3種(リング/挟み撃ち/3波)は具体的な半径・角度・体数・タイミングが
  仕様に無く、新規の湧き配置ジオメトリを要するためデータ雛形のみに留めた(数/襲撃/スパイクの3種は
  既存機構の再利用+仕様に明記された数値でそのまま配線)。設計チャットが上記R4-C節(v0.26.6)で
  6種全ての数値・座標式・実装形(`shallowExpression.ts`)を確定したため、この節の実装で解消する。
