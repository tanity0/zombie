...
- **【v0.26.8時点で改定・社長指示】憲法(`constitution.test.ts`)は廃止。** 第1〜5条を含め
  ...
  明記)。中身は現存する機構(countCap/ゾーン天井/床許可リスト等)の回帰ガードとしてそのまま有効
  だが、本書の裁定があればいつでも書き換え・削除してよい(「全条維持」の縛りではない)。
...
- AREA_WEIGHTとの関係: パンプキン/犬のTank化(自然湧きプール0)は**維持**。本台本が
  「意図した瞬間に落とす」新しい供給源になる(置き換えではなく、配役経路の追加)。
...
- 旧・憲法4条(初心者ゾーン不可侵)との関係: 本台本がエリア1に問題児を出すのは仕様(憲法廃止)。
  gatePressure側のゾーン天井(`ceilingForZone`)は§0重要ポイント4の安全弁として**残置**
  (=関所の配役圧はエリア0-1で従来どおり抑制されつつ、エリア台本の「たまに」だけが顔を出す)。
...
| R7 | エリア台本(§2)+ディレクター連携ゲート(§3の「たまに」発火条件) | **完了(v0.26.13)。gate中限定へ補正(v0.26.16)** |
