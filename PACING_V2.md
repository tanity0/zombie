...
- **【v0.26.8時点で改定・社長指示】憲法(`constitution.test.ts`)は廃止。** 第1〜5条を含め
  ...
  以降の設計・実装は下記§2「エリア台本」を含む本書の現行記述に従う(旧憲法の記述は
  歴史的経緯として残すが拘束力は無い)。`constitution.test.ts`自体の扱い(削除/書き換え)は
  エリア台本の実装バッチで対応する。
...
## 2. エリア台本(社長指示・v0.26.8〜書き込み中)
憲法廃止を受けて、エリア(距離帯)ごとに問題児の自然出現を直接指定する。**関所の台本ローテ
(R1)・浅いエリア代替表現(R4-C)とは別枠**(両者の関係・置き換えの要否は書き込み完了後に確認)。
...
| エリア4 | (社長指示: ここからは塩梅で任せる=実装判断可) |

- 上限(全エリア共通): パンプキン最大2/犬最大2/弾(plant)最大3の範囲に収める。
- 別枠(問題児とは別トラック): 叫び(screamer)最大1・ゴースト(ghost)最大2。
  3分経過で解禁、**ただしエリア3以降は開始直後から出現しうる**。
- ★未決事項候補(実装時に数値確定が必要・現時点ではSonnetは着手しない): 「たまに」の頻度/CD/
  確率、既存のAREA_WEIGHT(v0.25.1354でpumpkin/werewolfを全エリア0化=配役方式Tank化)との
  関係(このエリア台本が置き換えるのか併存するのか)、既存の問題児リフラクトリ15秒・boardDebt等の
  安全弁を維持するか。書き込み継続中のため、内容が出揃ってから設計チャットまたは社長判断で確定する。
...
| R7 | エリア台本(§2)+ディレクター連携ゲート(§3の「たまに」発火条件) | §2の数値確定待ち(「たまに」の頻度/CD等) |
