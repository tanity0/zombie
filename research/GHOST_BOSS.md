...
- **★originの機械化(監査指摘)**: 生成点は weaponUtils の4箇所(579/671/715/764)に散っている。
  付け忘れは「安全側フォールバック」が静かに隠すため、**不変条件テストで
  「isDirectGunWeaponKey が真の弾は必ず origin を持つ」を固定**する(生成点を数える代わりの機械化)。
  origin が無い弾(他所生成)は従来どおり抽選対象(反応可能扱い=安全側)。
