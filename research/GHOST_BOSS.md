...
- **★リーチ外からの近接はパリィされない(監査指摘1=意図として明記)**: 幻影のスイングは射程ゲート
  ...
  **構造的にパリィされない——これは仕様**(構えていない相手からは取れないのが自然。リーチ差の優位は
  ...
  (**絵の出ていない窓は読めない**——スイングの絵が窓の唯一の手掛かりで、圏外に見えない窓を
  置くと「赤くないのに当たる」型の不条理になるため。※v0.25.3665の射程ゲートは反撃スイング側の
  話で周期スイングとは別=矛盾の根拠にはならない・監査訂正)。
...
- **★飛翔時間の算出(監査指摘2)**: Projectile は発射点を持たず、createdAt は Date.now 基準
  ...
  ヒットストップの影響も受けない・掟にも抵触しない)。
  **★ガード(監査指摘・PHILLのスナップ弾が speed:0 で実在=gameStore 9176)**:
  `proj.speed ≤ 0` または dist が実質0の弾は**「瞬間着弾=見てから反応できない」→反応不可へ倒す**
  (パリィ抽選なし=必ず通る)。Infinity/NaN を比較に流さない(式の前で分岐)。
- **★ラリーの再打刻(監査指摘)**: reflectProjectile が弾を反転するたび、**origin を反射点
  (その時の弾の現在位置)へ再打刻する**。これで判定は常に「直近の飛翔距離」になり、
  近距離ラリーの打ち返しは反応不可=§2の狙い(近距離は打ち返されない)がラリーでも保たれる。
- **★originの機械化(監査指摘・三巡目で具体化)**: 生成点は weaponUtils の4箇所に加え
  **PHILL が gameStore 9176/9197 で直接組み立てる**など散在し、静的テストでは漏れを検出できない。
  よって**生成点では焼かず、storeへの挿入の合流点で必ず補完する**:
  addProjectile(gameStore 13554)と fireWeapons のバルク挿入(9583/9589)の3箇所が
  共通ヘルパ(例: `ensureProjectileOrigin(p)`=origin未設定なら挿入時の p.x/p.y を焼く)を通す。
  挿入時の座標=生成直後の銃口位置なので意味も正しい。**付け忘れが構造的に不可能**になり、
  「origin無し=反応可能」のフォールバックは他所生成の敵弾等にしか残らない。
  テストはヘルパ1本の単体+「挿入後の直接銃弾が origin を持つ」の統合1本で固定。
...
- 演出の編集対象は2箇所だけ(ハンドシェイク経由で1本に絞られている=ゲートの呼び出し7地点へ
  散らない): 近接=phantomTick.ts 446-449(consumePhantomParry の青白リング/バースト)・
  弾=useGameLoop.ts 10179-10180。§3の「出すもの」へ置き換え+triggerHitImpact を足す。
- テスト改訂: phantomTick.test.ts 185-215/228-245(ゲート単体=窓入力へ)・291-310(統合=先に
  ...
  **phantomParryProbe.test.ts(実在する・v0.25.3662)**=「窓中はパリィ・窓外は素通り」の2ケースへ
  書き換え。`Projectile`(types/game.ts:1460)に originX/originY を追加。
...
### 期待される体感(社長の読みどおり)
- 近接: 幻影のスイング周期820msのうち窓400ms=約半分(**パリィ成立後はCD1000msが窓を覆うため、
  連続で弾かれる局面の実効率はこれより下がる**——直後の窓のうち生きるのはCD明けの約220msだけ)。
  タイミングを読まない近接は最大でも約半々で弾かれる程度になり、
  **スイング直後の隙を狙えば確実に通る**(v2の「82%で問答無用に弾かれる」が消える)。
...
## v9後の裁定(2026-08-20・v0.25.3691)
- **近接は幻影の被弾無敵(i-frame)を無視する**(社長「相変わらず近接攻撃が効かない。近接攻撃は
  無敵時間無視で(近接にCDがあるので)」)。phantomHitGate ①を melee/counter はスキップ。
  銃連射が i-frame を張り続け、CD持ちの近接が常に吸われていたのが「効かない」の正体。
  近接の守りは②の窓パリィのみ。弾・遠隔は従来どおり無敵で弾く。テストは両方向を固定
  (遠隔=0のまま/近接=無敵中でも減る)。
- **PVP_DAMAGE_SCALE 0.1 → 0.2**(社長「ダメージ1/10を1/5に直す」)。派生: 弾パリィの打ち返しは
  ×REFLECT(10)×0.2=素の弾ダメの2倍で返る。

## ★v9実装中の未決(→全て決着済み)
- **成立演出に「Counter!」の文字(spawnCallout)を出すか(§3)** → **出す(社長裁定2026-08-20
  「幻影パリィにも文字出して」・v0.25.3684で実装)**。glow は検収監査(v0.25.3683)の判断で先行追加済み
  (主語を持たない光=色文法の構成要素・2箇所とも半径43=守護霊と同値)。これで幻影のパリィ成立演出は
  プレイヤー/守護霊のカウンター成立と**完全に同じ一式**(ring+burst+glow+Counter!+triggerHitImpact+SE)。
  編集箇所は§3と同じ2箇所(phantomTick.ts の consumePhantomParry・useGameLoop.ts の弾パリィ)。
