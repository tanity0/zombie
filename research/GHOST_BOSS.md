...
2. **被弾無敵=プレイヤーと同じ `INVULN_MS`(定数を直接参照・例外なし)**: 無敵中の打撃はHPダメージ0。
   ...
   ①gp-stagger(硬直・その間に1発は素で入る) ②体勢値**で出す——**体勢値はHP無敵のゲートを通さない**
   (体勢はカウンター専用の通貨。プレイヤー側に体勢値の概念は無いので、通しても非対称にはならない)。
3. **銃=台帳武器の実性能をプレイヤーの武器仕様ごと写す**: `createWeapon(snapshot.activeGunKey)` の
   **damage / cooldown / マガジン / リロード時間をそのまま**使う(handgun-t3=マシンピストルなら
   毎秒10発で撃つが、マガジンが切れたらリロードで止まる=プレイヤーと同じ息継ぎ)。予告なし・即発
   (プレイヤーの射撃に予告が無いのと同条件)。弾の見た目だけ**共通の赤二重丸**(CLAUDE.md「弾は
   全ボス共通」・打ち返し文法が効く)。gp-shot(windup型bossState)は廃止。
4. **HP=1200(叩き台・3000から変更)**: 無敵導入で通るダメージが毎秒1発に落ちるため、3000のままだと
   戦闘が桁で伸びる(監査重大5)。数値は実機で社長が調整する前提の初期値。

## 監査18件の反映(設計の穴の解消)
### 対幻影ダメージのゲート(監査重大2/3の解)
- **合流点は1箇所ではない。以下の7系統を名指しで全てゲートする**(唯一のヘルパ
  `phantomHitGate(enemy, amount, source)` を葉(phantomScript.ts)に置き、全site が呼ぶ):
...
- **ゲートの返り値は「実効ダメージ+副作用可否」**: 無効化(無敵/パリィ)した打撃は
  **ノックバック・100ms固め・吸血回復・救難信号・ナイフコンボ加算・damageDealt を発火させない**
  (`meleeHitEnemyIds` から除外)。体勢値だけは通す(上の裁定2)。
- 受け入れ条件: **7系統それぞれに「無敵中は0」のユニットテスト or 結合テスト**(1経路でも素通りが
  あれば「無敵が存在しない」に等しい・監査の予言を機械で塞ぐ)。
...
- 銃撃は bossState 機械から**独立**させる(プレイヤーが移動しながら撃つのと同じ層)。
  技の抽選(nextMoveAt)は**近接/一閃だけ**になり、`pickPhantomMove` は
  近(縁基準)=gp-melee / 中(一閃射程内)=ISSEN_MID_CHANCE で gp-issen(外れたら技なし=射撃継続)。
  **`decision.action==='none'` でも中距離なら抽選を回す**(旧: actionが立たないと入口ごと閉じた)。
- 距離の単位は**縁距離(enemyRangeRect最近点)に統一**(監査小15: preferredDistは縁基準)。
### 弾回避(監査・問い1-③/小16/小18)
...
### 近接の予告(監査中10)
- windup 250ms(叩き台)。**赤帯はカラオケ式(prog=1で完成)ではなく、windup開始で即フル表示**に変える
  (250msでは「完成した瞬間に着弾」になるため)。受け入れ条件: **社長が実機で「見てから取れる」こと**
  (取れなければ windup を 350-400ms へ上げる調整余地を明記)。
### gp-shot撤去の波及(監査中12の訂正)
- 撤去対象: `types/game.ts` の bossState union(gp-shot 3州)/ phantomScript の
  **windup/gap/recover のみ**(damage/speed/size/range は銃撃ミラーが使うので**残す**)/
  `lastRangedShotAt` の死んだ書き込み / phantomTick.test の①(中・遠でgp-shotを期待する4本)と
  ⑤(shotsRemaining)。**counterReach の gp-shot 宣言は存在しない=撤去作業なし**(空作業を書かない)。
### テスト(監査中4/中13)
...
- 銃の絵(監査小17): 武器スプライトの統一型(PACING_PUZZLE §7-15)で **rogue の手元に台帳の銃**を
  出し、発射時マズルフラッシュ+反動(戻りease=慣性MUST)。
...
## 実装地図(Opusバッチ1本・大)
1. phantomScript: phantomHitGate(無敵/パリィ/副作用可否・rand注入)+parry/melee/銃の定数整理
2. gameStore: 7系統のゲート呼び出し+副作用の抑止(meleeHitEnemyIds除外)+gpParriedAt
3. phantomTick: 銃撃ミラー(createWeapon実性能・マガジン/リロード)/一閃の独立抽選/
   knockback凍結の除外/パリィ消費→即反撃/gp-shot州の撤去
4. 描画: 手元の銃+マズル+反動/パリィ青白スパーク/無敵中の白点滅(ヒットSEなし)/赤帯の即フル表示/
   モーション整理
