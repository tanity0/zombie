...
### ★未決事項(社長裁定がほしい・実装は設計書どおりで着地済み)
- **速度**: 設計書の `speed=87(プレイヤー基準)` を ENEMY_STATS へそのまま入れたが、**実効値は
  ×`ENEMY_SPEED_MULT`(2/3)= 58** になる(固定強度タイプなので areaSpeed は 1)。
  「プレイヤーと同じ速さ」を実効値で満たすなら生値を **130 前後**にする必要がある
  (前例: トールが同じ理由で生値163を明示指定している)。**推薦は実機で社長が見てから決める**——
  幻影は間合いを取る相手なので、プレイヤーより少し遅い方が読める可能性もあるため。
