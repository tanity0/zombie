...
## ★v9実装中の未決
- **成立演出に「Counter!」の文字(spawnCallout)を出すか(§3)**。
  ※検収監査(v0.25.3683)で未決を1点に絞った: **`spawnGlow` は主語を持たない光**で、
  プレイヤー成立(combatTick 284-289)・守護霊成立(ghostCounter の青カウンター層)の両方に
  入っている色文法の構成要素=裁定不要と判断し、**検収で追加済み**(2箇所とも半径43=守護霊と同値)。
  残る未決は callout のみ: これは「幻影がプレイヤーを弾いた」時の絵なので、文字を出すと
  **プレイヤーが取ったように見えないか**が判断材料(社長裁定案件)。現状=**出していない**。
  出す判断になったら編集箇所は§3と同じ2箇所(phantomTick.ts の consumePhantomParry・
  useGameLoop.ts の弾パリィ)。
