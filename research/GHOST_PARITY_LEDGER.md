...
## ★仕様: 守護霊カウンターの判定時置換ミラー(社長裁定2026-08-27・設計チャット直轄)
...
守護霊の成立 = 同じ式で「プレイヤーの体」→「守護霊の体(playerHitbox(ghost))」、
「プレイヤーの窓」→「守護霊の窓(下記)」に置き換えたもの。
**成立域の図形・成立州・判定関数はプレイヤーが使うものをそのまま使う(inCounterReach /
counterReachShapeFor / HIDDEN_COUNTER_ACTIVE_STATES / BOUNTY_ACTIVE_COUNTER_STATES等)。**

### 窓(構え)の定義変更(ghostCounter.ts)
- **積む時刻 = 振り始め**(gPendingSwingAt打刻の瞬間=カウンター意図が立った瞬間)。
  旧: 前隙解決時(+200ms)。プレイヤーの「押した瞬間から受付」(隻狼型v3943)と同じ。
- **長さ = `COUNTER_ACCEPT_MS`(300ms・gameStoreの定数を参照。手写し禁止)。** 旧TTL150は廃止。
- **被弾で窓が閉じる**(ghost.lastHit > 窓開始)は維持=プレイヤーと同じ。?lastcounter=1の復帰も維持。
- **請求側の位置ゲート(矩形重なり)は削除**: 成立の瞬間に成立域判定へ守護霊の矩形を入れるので、
  請求側の独自位置条件は二重かつ過剰(旧: スイング後に離れた成立を防ぐためのもの)。
  `inAttackZone` optsも不要になる(爆風は爆風の幾何が、接触は接触判定が位置の真実)。
- 最新1件のみ・対象ボスのみ・1成立で消費、は従来どおり。

### 成立地点(全系統・プレイヤーの判定と同居・プレイヤー優先は現行どおり)
プレイヤー不成立のフレームだけ守護霊を同じ式で判定する:
1. **裏4+トール ACTIVE図形**(useGameLoop hiddenブロック): hiddenReachOverlapNow と同じ判定を
   守護霊の矩形+守護霊の窓で再評価(旧「isCounterOpportunityNow && 請求あり」ゲートを置換)。
   成立→既存 thorCounterHit / hiddenBossCounterHit(GhostCounterFire)。
2. **裏4+トールの判定時置換カプセル技**(一閃/突き/払い/突進斬り抜け/ミーミルレーザー=
   applyGhostAllyCapsuleHit の呼び出し5箇所): カプセルが守護霊に当たった瞬間、窓が生きていれば
   **ダメージを出さずに置換成立**(プレイヤーの各州ハンドラ内の判定時置換の写し)。
3. **賞金首**(bountyTick カウンターブロック): inCounterReach(プレイヤーと同じ図形)を守護霊の
   矩形+窓で再評価(現行の「請求があれば成立」を置換)。tryMovingCounter(移動後図形)にも
   同じ守護霊分岐(プレイヤー不成立時のみ)。
4. **天使6**(angelBossTick takeGhostAngelCounter): プレイヤーの成立式と同形へ
   (成立州+成立域をプレイヤーの判定と同じ関数で守護霊矩形評価。「請求存在」だけのゲートを置換)。
5. **城ボス**: 爆風(combatTick 請求消費)と接触パリィ(applyGhostBossParry)は既に「当たる/触れる
   瞬間」型=**窓の定義変更が効くだけ**(ゲートの形は維持)。
6. **汎用ボス接触**(useGameLoop 敵→summon接触ブロック): 交戦対象ボス(isEngageableBoss)の本体
   接触が守護霊に入る瞬間、窓が生きていれば置換(プレイヤーの接触カウンターの写し)。
   プレイヤー側と同じ除外(shouldSkipBossContactParry=専用フル報酬カウンターへ委ねる技)を通す。
...
### 構え(ghostDriver)の狙い直し
窓が「攻撃の解決時刻」に生きていなければ成立しないため、駆動側もプレイヤーの
「赤が消え切る瞬間に押す」を写す:
- **溜め(windup)中に構えられるようにする**: 機会条件へ「着弾時刻の分かる予告中」
  (bossStateUntil/aiPhaseUntil を持つwindup州・予告台帳の対象)を追加。
- **振る時刻=着弾予定時刻から逆算**: 既存A-2の着弾逆算(giantbat限定)を着弾予告全般へ一般化。
  窓300msが着弾時刻を覆うように振る。反応の遅い霊ほど逆算が甘い(ghostAimLeadMsの個性)は維持。
...
- **「ボスの技が守護霊にダメージを与えるようにする」拡張はしない**(当たらない技は当たらないまま。
  成立域の再評価だけを足す)。ヘイトで守護霊を狙った技の実ダメージ化は別案件。
- **弾反射(ghostCounterWindowEnd)は別機構のまま触らない。**
- UI/表示の追加はしない(窓リング等の既存表示は現状維持)。

### 受け入れ条件
1. 賞金首・トール・裏3・天使・城ボスとの戦闘で、守護霊のカウンター(青Counter!)が実機で
   観測できる頻度に戻る(ゴール①)。
2. 通常戦・プレイヤーのカウンター挙動に変化が無い(ゴール②=バグを出荷しない)。
3. ユニットテスト: 窓の意味(振り始め+300ms/被弾クローズ/1成立1消費/COUNTER_ACCEPT_MS参照)、
   賞金首・裏ボスの守護霊成立(図形+窓)、idol対称(構えない)、着弾逆算の一般化。
