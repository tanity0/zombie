...
4. **天使6**(angelBossTick): プレイヤーの成立式と同形へ——
   ...
   生矩形判定を守護霊で) && 窓`(M5)。
   **★検収1巡(重4)=裁定済み(社長2026-08-27「はい」)**: 実は「体の重なり」はプレイヤーの写しとして
   **誤り**だった——プレイヤーは振り技を**振りの帯・円(実ダメージと同じ図形)の中**で取っている
   (体の重なりで取るのは溜め/硬直の体当てだけ)。よって守護霊も**各ハンドラの同じ図形**で
   判定時置換する(v0.25.3977で実装: ミゲル払い/縦払い/斬り抜け・ウリ大薙ぎ/振り下ろし/踏み込み突き・
   スリエル環射出×2本/回転斬/本体薙ぎ)。体の重なり(takeGhostAngelCounter)は体当たり系の担当として残置。
   アクラシエルの棘/爆発・ジブリルのランタン等は機会州リスト外=守護霊は構えない(従来からの狭い側・
   v3131の★未決のまま)。
