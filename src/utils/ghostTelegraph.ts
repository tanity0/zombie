...
// ---- §16 雑魚の「詰めさせない技」の相(v0.25.4453で分類) --------------------------------------
//
// ★**この台帳はボスの予告が対象**(冒頭の説明どおり)。§16(PACING_PUZZLE.md)で雑魚に足した相は
// **ボスの技ではない**ので、どれも `coverage: 'none'` = **この表は何も足さない**。
// ⇒ **現在の挙動と同じ**(台帳に無い=誰も避けていない)。ここでやっているのは
// 「分類されていない状態を無くす」ことであって、**避け方を変えることではない**。
//
// ★なぜ 'none' で済まさず1件ずつ書くか: この網は「**新しい技を足したらここで落ちる**」ための物で、
// §16のバッチは**15個まとめて足して、1個も登録せずに通っていた**(v0.25.4453で発覚)。
// まとめて除外すると次も同じことが起きるので、**相の性質ごとに分けて理由を書く**。
put(LEDGER, [
  // ゾンビ(§16-3): 赤2連の前後。
  'z-lunge-in',   // 踏み込み(移動)
  'z-stagger',    // よろけ(硬直)
  'z-recover',    // 技後の硬直
  'z-retreat',    // 後退(移動)
  'z-red-pause',  // 赤の点滅中の停止
  // コウモリ(§16-1)
  'b-orbit',      // 円運動(移動)
  'b-release',    // 掴みの解除〜離脱(移動)
  // スケルトン(§16-2)
  's-crouch',     // しゃがみ(溜め。図形はまだ無い)
  's-arc',        // 弧を描いて回り込む(移動)
  's-recover',    // 技後の硬直
  's-retreat',    // 後退(移動)
], {
  coverage: 'none',
  note: '§16 雑魚の相のうち**移動・硬直・溜め**。当たり判定を持たない=避ける図形が定義できない。',
});
put(LEDGER, [
  // ★当たり判定を持つのはこの4つだけ(踏み込んで体を当てる=判定は「敵の体とプレイヤーの重なり」)。
  'b-windup', 'z-bite1', 'z-bite2', 's-bite',
], {
  coverage: 'none',
  note: '§16 雑魚の噛みつき。判定は**敵の体そのもの**(専用の図形を持たない=「絵と判定が同一」)。'
    + '**雑魚を避け対象に入れるかは未裁定**——入れると守護霊が雑魚から逃げ回る別の挙動になるため、'
    + '現行どおり拾わない。裁定が出たら circle-self へ変える(半径は接触距離の複製値)。',
});

put(LEDGER, [
  // ★リッチ「転移噛み」(§16-C・v0.25.4484)。**上の4つとは理由が違うので分けて書く。**
  'lich-blink',
], {
  coverage: 'none',
  note: '§16-C リッチの転移噛み。★**この技だけは避ける図形が定義できる**——判定は敵の体ではなく'
    + '**発火時に焼いた点を中心とする円**(`lichBlinkAtX/Y` + `LICH_BLINK_RADIUS_PX`=40)で、'
    + '`isInBiteCircle` が読む。つまり `circleThreat` へそのまま渡せる。'
    + 'それでも none にしてあるのは、上の噛みつき4件と**同じ未裁定**(**雑魚を避け対象に入れるか**)に'
    + '属するため——1件だけ拾うと「リッチからだけ逃げる守護霊」になり、挙動が揃わない。'
    + '★**裁定が出たら、この行が最初に circle へ変わる**(図形が既にあるので複製値を足すだけで済む)。',
});
