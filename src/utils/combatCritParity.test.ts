...
//  - ボス対象: crit=trueがdamageEnemyへ渡り、①の中央適用(bossSlowUntil半減窓+bumpBossCrit紫蓄積)が乗る。
...
describe('CRIT-UNIFY §9.3: 接触パリィ(dashParried)の反撃はcrit=true', () => {
  it('ボス(giantbat)への突進パリィ反撃はbumpBossCritへ乗る(bossCritCountが増える)', () => {
    ...
    // 反撃が確定クリになった=bumpBossCritが走ってカウントが1増える(旧実装は0のまま=クリ非登録だった)。
...
describe('CRIT-UNIFY §9.3: ブラストパリィ(パンプキン着地爆発)の反撃はcrit=true', () => {
  it('ボス(pumpkin)へのブラストパリィ反撃はbumpBossCritへ乗る', () => {
    setupPlayerAt(0, 0, 400);
    const player = useGameStore.getState().player;
