...
// blown/downed は爆撃(演出仕様v3.1)の被弾: **ノックバックしながらしゃがむ**(社長指示2026-08-29
// v4054「倒れるのではなく、ノックバックしながらしゃがむ、で」。絵はpixi側がしゃがみ絵
// soldier-watch-0 に切替)。downedは終端=しゃがんだまま(「起こさないで」の裁定は継続)。
// 画面外へ抜けたら右から新規walkerとして補充される(reenter参照)=シーンの人数は痩せない。
// 'getup' はv4039〜4046の起き上がり演出の名残(現在はどこからも遷移しない)。
...
export const stepEndingSoldier = (
  ...
  switch (s.phase) {
    ...
    case 'downed': {
      // 終端=しゃがんだまま(社長指示「起こさないで」+v4054「ノックバックしながらしゃがむ」)。
