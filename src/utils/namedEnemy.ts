...
/**
 * 保存済みの宿敵メタの浄化(v0.25.3694)。除外型(幻影等)が過去の穴で保存されてしまった端末を
 * 読込時に救済する(nullなら宿敵不在に戻る)。redNightは保存時の状態と無関係なのでfalseで判定。
 */
export const sanitizeNamedFoe = <T extends { type: EnemyType }>(m: T | null): T | null =>
  m && !isPromotionExcluded(m.type, false) ? m : null;
