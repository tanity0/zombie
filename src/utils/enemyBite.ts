...
/**
 * ★溜め中の点滅の色(v0.25.4349・社長指示2026-09-16
 * 「**ダッシュ噛みつき発動のタイミングで赤点滅させてゾンビ**」)。
 *
 * 既定は**紫**(`counterable: false` と一対=CLAUDE.md 色と形の文法②「紫=カウンターできない攻撃」)。
 * **ゾンビだけ赤**にする。文法上これが成り立つ理由を明示しておく:
 *   文法①の赤は「**カウンター/回避の対象**」で、**回避を含む**。ゾンビのダッシュ噛みつきは
 *   踏み込みの向きを発火の瞬間に焼く=**追尾しない=横へ避けられる**(§12の裁定)。
 *   つまり「赤いのに何もできない」ではなく「赤い=避けろ」という読みになる。
 * ★ただし**カウンターは依然できない**(`counterable` は false のまま=判定は一切変えていない)。
 *   「赤はカウンターできる印」と受け取られるのが困る場合は、ここを紫へ戻すか、
 *   ゾンビの噛みつきをカウンター可にするかの**どちらかへ揃える**(社長裁定待ち)。
 */
export const biteBlinkTintFor = (type: EnemyType): number =>
  type === 'zombie' ? 0xef4444 : 0x9333ea;
