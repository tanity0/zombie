...
export const generateUpgradeOptions = (player: Player): UpgradeOption[] => {
  ...
  // スキル装備は「固有スキル(クラス標準) + 新規取得スキル1個」まで。刀/村雨/ダンスフロアは排他=この上限から除外。
  // 固有スキルは最初から所持しているので上限カウントから除く。新規取得は1個まで(既所持の昇格は常に可)。
  const EXCLUSIVE_SUBS: string[] = ['katana', 'murasame', 'shijin'];
  const innateSub = classSubWeaponFor(player.characterClass);
  const ownedSkillCount = player.subWeapons.filter(k => !EXCLUSIVE_SUBS.includes(k) && k !== innateSub).length;
