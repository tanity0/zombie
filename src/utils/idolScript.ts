...
export const IDOL_TUNING: IdolTuning = {
  ...
  // ★社長指示2026-09-11「アイドルも地味だわ。これも見直して激ムズ、派手に」。
  // 扇3本×開き0.14rad = 全角**16度**で、扇に見えず「3発まとめて1方向」でしかなかった。
  // 主題は「近づくほど安全」なので、**遠距離の圧を上げるのが正しい強化方向**(近距離技は据え置き)。
  // 扇は遠いほど広がるので、増やしても**詰めれば1本しか当たらない**=主題を壊さない。★叩き台。
  fanCount: { p1: 5, p2: 8 },  // 連射の扇の本数(旧3/5)
  orbCount: { p1: 3, p2: 5 },  // 追尾弾の数(旧2/3)
  ...
  shape: {
    ...
    fanSpreadStep: 0.17,  // 1本あたりの開き角(rad)。5本で全角0.68rad(39度)/8本で1.19rad(68度)
  ...
  // ★v0.25.4204: P1から4段(旧3)。ストリングが途切れないほど圧が続く=「地味」の解消。
  stringLen: { p1: 4, p2: 4 },
  strings: [
