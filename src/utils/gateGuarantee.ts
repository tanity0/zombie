...
// 破ってはならない(この関数の外=呼び出し側の責務): 実効天井/同時数キャップ/リフラクトリ15秒。
// 26系の実効天井は台本maxRung由来のみ。`?v2=0`旧経路だけゾーン天井も掛ける。
// 実効天井で解禁されない型は`allowedAtCeiling`に含めないことで対象外にする。キャップ満杯時の
// 「空き待ち」も呼び出し側がpendingとして毎フレーム再評価する(このモジュールは「今投入すべきか」の判定のみを持つ)。
//
...
export interface GateGuaranteeInputs {
  ...
  allowedAtCeiling: ProblemChild[];            // 実効天井で解禁されている問題児(allowedProblemChildren(ceiling, order)の結果)
