...
//
// 7分アークを 余裕(buildup) ⇄ 関所(gate) で刻み、屋外の通常湧きに使う。
// 城ボスは独立した5:00出現で、このフェーズ時刻自体は変更しない。
...
// ★旧「出だしの体数抑え」(OPENING_COUNT_RAMP・社長指示2026-09-17)は
// PACING_PUZZLE.md §17-11(監査A-7)で廃止した。理由: ウェルカム台本(§17)がステージごとの出だしの
// 顔ぶれ・体数を持つようになったので、ステージ共通の数字による二重の抑えは不要になった(残すと
// ウェルカム明けの70秒間、enemyCountCap/computeDirCountCapが再び2→4→6を掛けてしまう=二重
// リミッター)。出だしの抑えは今は §17 の台本そのものが表現する(§17-6)。
