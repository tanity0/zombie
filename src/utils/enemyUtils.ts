 ...
 *
 * ★v0.25.3921(社長裁定「まず推薦で入れてみて」): **本物のボスも噛みつきに乗る**ようになった
 * (技の合間のつなぎ=CD長めの近接台本)。よって噛みつきの除外は下の `isBiteExemptType`
 * (賞金首4種 / 死神 / 幻影)だけになり、この述語は**噛みつきの数値(硬直=CD)をボス用に切り替える**
 * ために使う。
 */
/**
 ...
 */
/**
 * ★★**この表は「噛みつき台本に乗せない型」であって、「触れたら痛い型」ではない**(v0.25.4646 で是正)。
 * 社長質問2026-09-25「**幻影も触れたら痛いの？**」で分かった——**幻影は触れても痛くない**。
 * 接触ダメージの門(`combatTick.applyContactDamage`)は `isBiteSubject` の**手前で**
 * `isGuardianPhantom` を明示 return しており、`ENEMY_STATS['guardian-phantom'].damage` も **0**
 * (research/GHOST_BOSS.md「幻影は接触では削らない」)。幻影が削るのは**自前の技だけ**。
 * ⇒ **平時に触れて痛いのは 死神 と 使者 の2型**。v0.25.4643 の報告「3型」は誤りだった
 *   (述語を1つだけ見て「門」を語らない)。
 *
 * ★**再確認された裁定(2026-09-25・社長「推薦で」)**: 死神・使者の接触ダメージは**このまま残す**
 * ——上に書かれた存在意義(「触れたら終わり」「捕まると一撃死」)そのものなので。
 * 読みづらさが問題になった時に足すのは**予告**であって、接触ダメージを外すことではない。
 * ※削岩型は最初から入っていない(社長が食らっていたのは溜め0.3秒の噛みつき。v0.25.4645 で600msへ)。
