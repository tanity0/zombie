...
export const ALLOWED_MOVE_CHAINS: Readonly<Record<string, string>> = {
  ...
  'backroll -> quickblades':
    'ラフィのロール台本「バックロール→骨刃2連射」(社長指示v0.25.3592)。ロールは**攻撃ではない移動**で、'
    + '持ち時間(roll.rollMs)を使い切った時にだけ次へ渡す=途中で切っていない(br-roll等と同じ型)。',
  'mk-backroll -> mk-boom':
  ...
  // ★実戦の状態へ常時接続する(ボス・ガントレット)前に足した既知の設計連携2件。
  // 未申告のまま繋ぐと**最多枠(城ボス7枠)とミゲルで毎戦誤検知**が出て、本物の違反が埋もれる。
  // ※キーは**状態名ではなく parseMovePhase 後の技名**(接尾辞 -windup/-active/-recover を落とした形)。
  'g-quad-charge -> g-quad-breath':
    '城ボスの四連突進「3回突進しきってから氷結の吐息」(gameStore.ts の g-quad-charge)。'
    + '突進は**3回を数え切った時にだけ**次へ渡す(giantQuadDashComplete)=途中で切っていない。'
    + '学習装置として回数を3固定にした1つの連続した技。',
  'harai -> tate':
    'ミゲルの「払い→縦払い」(angelBossTick.ts)。払いは**持ち時間(bossStateUntil)を使い切った時にだけ**'
    + '次へ渡す(カウンターで中断された時は chase へ抜けてここを通らない)=途中で切っていない。'
