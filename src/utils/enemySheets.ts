...
export const ENEMY_WALK_SHEETS: Readonly<Record<string, number>> = {
  ...
  // ★グレン形態1の浮遊移動(社長支給2026-09-26「グレンの浮遊移動」)。16コマ(支給 6416×258 → 上の空き2行だけ
  // 落として **401×256**)。常駐 **6.57MB**=遅延組(下の常駐表)。高さ256=台帳のボス帯(実走で画面211px)。
  // ★縮小なし: 2×2一致率は28%と高めだが**位相を1pxずらしても28%のまま**=引き伸ばしの跡ではなく、
  //   32色で塗った暗い外套の平らな面(引き伸ばしなら位相0だけが100%近くになる)。半透明0%・左向き(立ち絵と同じ)。
  // ★倍率: 立ち絵(256×194)へ重ねて 1.24〜1.32(4コマ)。枠の比 256/194=1.32 なので自動の枠合わせだと約3%大きく出る
  //   =1割未満なので `ENEMY_WALK_BODY_H` には載せない。足元(外套の裾)は全コマ最下行。
...
export const SHEET_RESIDENCY: Readonly<Record<string, SheetResidency>> = {
  ...
  // グレン形態1。浮遊6.57+攻撃1.30+跳び2.21=**10.08MB**。ステージ7でしか出ず、出現にカットインを挟むので遅延。
  'glen-boss': 'deferred',
