...
export const ENEMY_WALK_SHEETS: Readonly<Record<string, number>> = {
  ...
  // 11コマ(支給 2838×258 → 余白を切って **254×256**)。常駐 **2.73MB**。切る矩形は全コマ共通(x2-255 / y2-257)。
...
export const ENEMY_JUMP_SHEETS: Readonly<Record<string, JumpSplit>> = {
  ...
  // ★城ボス1(搬送体・型は `giantbat`)11コマ(支給 1375×130 → 余白を切って **121×128**)。常駐 **0.65MB**。
  // ★★**解像度を半分にして出し直してもらった版**(v0.25.4576)。旧版は 243×256×16コマ=3.80MB で、
  //   **画面132pxに対して絵が243ドット=1.84ドット/px**=毎フレーム46%のドットを捨てていた。
  //   新版は **0.92ドット/px=1ドットも落ちない**(むしろ1.09倍に拡大されて出る)。常駐は **1/5.8**。
  // ★**コマ数が16→11に減っている**(変換側で間引かれた。旧コマ 1/5/9/10/14 が無い)。
  //   残った11コマは旧の 0,2,3 / 4,6,7,8 / 11,12,13,15 に対応するので、区間は **4/7/5 → 3/4/4**。
  // 読み: **0〜2=立ちから沈み込む** / **3〜6=踏み切り〜滞空〜落下** / **7〜10=接地〜立ち直り**。
  // ★7コマ目(区間の先頭)が**接地**=判定の着地(爆風)と同じ瞬間に出る。
  ...
  'giantbat': { crouch: 3, air: 4, land: 4 },
};
