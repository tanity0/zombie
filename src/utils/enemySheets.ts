...
export const ENEMY_SWEEP_SHEETS: Readonly<Record<string, SweepSplit>> = {
  ...
  // ★グレン形態1の攻撃(社長支給2026-09-26「グレンの…攻撃」)。9コマ(支給 3150×258 → 上の空き2行だけ落として
  // **350×256**)。常駐 **3.23MB**。縮小なし(2×2一致率35%だが位相で変わらない=平らな面)・半透明0%・左向き。
  // 読み: 0=立ち絵と同じ構え / 1〜4=腕を引いて触手を溜める / **5=触手を左へ突き出し切る(左端が枠の2px)** /
  //   6=触手を振り回す / 7,8=触手が周りへ広がって収まる。⇒ **5(溜め)/ 2(当たり)/ 2(戻り)**。
  // ★城ボス標準技+爪痕系(talon/boon/reach)の3相を全部これで描く(城ボス1・5と同じ扱い)。
  //   **跳ぶ技(`g-jump-`/`g-trijump-`/…)は `GIANT_JUMP_TECHS` で最初から外れる**=跳び/踏み潰しの絵が来るまで従来どおり。
  // ★倍率: 0コマ目を立ち絵へ重ねて 1.22(IoU 0.76)。枠の比 1.32 との差は約8%=1割未満なので `bodyH` は付けない。
  'glen-boss': { windup: 5, active: 2, recover: 2 },
};
...
export const SHEET_RESIDENCY: Readonly<Record<string, SheetResidency>> = {
  ...
  // グレン形態1。浮遊6.57+攻撃3.23=**9.80MB**。ステージ7でしか出ず、出現にカットインを挟むので遅延。
  'glen-boss': 'deferred',
