...
export const ENEMY_SWEEP_SHEETS: Readonly<Record<string, SweepSplit>> = {
  ...
  // ★重み(表示の長さの比。実効 溜め1000ms / 突き183ms / 戻り333ms に配る):
  //   溜め 0:72ms 1:84 2:108 3:133 4:157 5:193 6:217 **7:36** ——引き込みは**減速しながら**溜めへ収まり、
  //     最後に出はじめの1枚を弾く(慣性MUST。等分の143ms均等だと「同じ速さで畳んで同じ速さで出す」になる)。
  //   突き **8:57ms 9:126** ——突き切った絵を先に見せ、伸び切りで止める。
  //   戻り 10:94 11:61 12:72 13:106 ——出したまま小刻みに震え、最後に引き戻しへ入る(均等にしない)。
  'driller-common': {
    windup: 8, active: 2, recover: 4,
    weights: [1.2, 1.4, 1.8, 2.2, 2.6, 3.2, 3.6, 0.6, 1, 2.2, 1.7, 1.1, 1.3, 1.9],
  },
