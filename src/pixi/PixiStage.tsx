...
const PixiStage: React.FC<PixiStageProps> = ({ width, height }) => {
  ...
  // 変な切り抜きになる)。preserveDrawingBuffer無しでもrender直後の同期toDataURLは有効。
      ...
      void (async () => {
        ...
        const [labGround, s3Far, s3Ground, s3Horizon, s3Near, s1Near, s2Far, s2Near, s4Far, s4Front, s4Ground, s4Horizon, s3Front, s5Far, s5Horizon, s5Near, s5Front, s5Ground, tutFar, tutGround, tutFlow1, tutFlow2, tutRocks, tutNearRocks, tutFrontRocks, s7Far, s7Clouds, s1Sky, s1Castle, s1Moon, mansionBack] =
        ...
        scene.setFarBackdropTexture('mansion', mansionBack); // ステージ6(洋館)の遠景(仮=壁の一枚絵。corridorMode時にpixiSceneが'mansion'を選ぶ)
