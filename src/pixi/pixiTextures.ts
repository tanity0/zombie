...
import { ENEMY_SHOT_SHEETS, shotSheetName, ENEMY_IDLE_SHEETS, idleSheetName, ENEMY_JUMP_SHEETS, jumpSheetName, ENEMY_SWEEP_SHEETS, sweepSheetName } from '../utils/enemySheets';
...
export const ensureTextures = (): Promise<void> => {
  ...
  loading = (async () => {
    ...
    const standalone: { name: string; scaleMode?: 'nearest' | 'linear' }[] = [
      ...
      { name: 'bat-lantern', scaleMode: 'linear' },
      { name: 'fx/bat-slam-0', scaleMode: 'linear' },
      { name: 'fx/bat-slam-1', scaleMode: 'linear' },
      { name: 'fx/bat-slam-2', scaleMode: 'linear' },
      { name: 'fx/bat-slam-3', scaleMode: 'linear' },
      { name: 'fx/bat-slam-4', scaleMode: 'linear' },
      { name: 'fx/bat-slam-5', scaleMode: 'linear' },
      { name: 'fx/bat-slam-6', scaleMode: 'linear' },
      { name: 'fx/bat-slam-7', scaleMode: 'linear' },
      { name: 'fx/bat-slam-8', scaleMode: 'linear' },
      // スケルトンの爪の引っ掻き痕(4コマ)とVFX(5コマ)。社長支給2026-09-18。紫版は色相だけ回した同じ絵。
      { name: 'fx/skel-claw-0', scaleMode: 'linear' },
      { name: 'fx/skel-claw-1', scaleMode: 'linear' },
      { name: 'fx/skel-claw-2', scaleMode: 'linear' },
      { name: 'fx/skel-claw-3', scaleMode: 'linear' },
      { name: 'fx/skel-claw-p-0', scaleMode: 'linear' },
      { name: 'fx/skel-claw-p-1', scaleMode: 'linear' },
      { name: 'fx/skel-claw-p-2', scaleMode: 'linear' },
      { name: 'fx/skel-claw-p-3', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-0', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-1', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-2', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-3', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-4', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-p-0', scaleMode: 'linear' },
      { name: 'fx/skel-claw-fx-p-1', scaleMode: 'linear' },
      ...
      // ハンターの棺桶(社長支給2026-09-18)と、振りの斬撃10コマ+地面の余韻5コマ。
      { name: 'hunter-coffin', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-0', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-1', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-2', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-3', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-4', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-5', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-6', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-7', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-8', scaleMode: 'linear' },
      { name: 'fx/coffin-swing-9', scaleMode: 'linear' },
      { name: 'fx/coffin-slam-0', scaleMode: 'linear' },
      { name: 'fx/coffin-slam-1', scaleMode: 'linear' },
      { name: 'fx/coffin-slam-2', scaleMode: 'linear' },
      { name: 'fx/coffin-slam-3', scaleMode: 'linear' },
      { name: 'fx/coffin-slam-4', scaleMode: 'linear' },
      ...
      { name: 'fx/melee-arc-2', scaleMode: 'nearest' },
      { name: 'fx/melee-arc-3', scaleMode: 'nearest' },
      { name: 'fx/melee-arc-4', scaleMode: 'nearest' },
      { name: 'fx/melee-arc-5', scaleMode: 'nearest' },
      { name: 'fx/melee-arc-6', scaleMode: 'nearest' },
      ...
      // v0.25.3122(社長指示・**素材は後で支給**): 虚無の三唱の3コマ。赤い判定円(直径520px)いっぱいに
      // 貼るので、縮小のチラつきを避けて linear。**口だけ先に開けておく**=ファイルを
      // `public/sprites/fx/nihil-1.png`(〜3)へ置いた瞬間に出る。未配置の間はロードに失敗して
      // getTexture=null → 描画を丸ごとスキップする(ローダは1枚ずつ失敗を握りつぶす設計)。
      { name: 'fx/nihil-1', scaleMode: 'linear' as const },
      { name: 'fx/nihil-2', scaleMode: 'linear' as const },
      { name: 'fx/nihil-3', scaleMode: 'linear' as const },
    ...
    // 引き雲(遠景層)用の上下端フェード版(社長報告v0.25.3020「遠景近くに変な線」対策)。
    ...
    // 線形αフェードした別名テクスチャを焼いて登録する。
    // ★v0.25.3049(社長特定「これ全ステージだ。遠景の雲が犯人です」): 下端も同じ機構で線が出ていた。
    // 下端の透明は4行しかなく(下端付近でも半不透明の雲ドットが残る)、遠景層の帯の下端は地平線
    // (farH)ちょうどに来るため、雲が下端でスパッと切れる+ぼかしの矩形クリップで**地平線に横一線**が
    // 全ステージで出る。下から18%も線形αフェードして両端とも空に溶かす(テクスチャ名は従来のまま)。
    const loadTopFadedCloud = async () => {
      try {
        ...
        // ★v0.25.3049: 下端も18%を線形フェード(地平線に乗る帯の下端の横一線対策・上と同じ機構)。
        const fadeRowsB = Math.max(1, Math.round(h * 0.18));
        for (let y = h - fadeRowsB; y < h; y++) {
          const k = (h - 1 - y) / fadeRowsB;
          for (let x = 0; x < w; x++) {
            const i = (y * w + x) * 4 + 3;
            d[i] = Math.round(d[i] * k);
          }
