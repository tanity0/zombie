...
const OpeningScene: React.FC<{ onDone: () => void; startAtShoot?: boolean }> = ({ onDone, startAtShoot }) => {
  ...
  useEffect(() => {
    ...
    Promise.race([Promise.all(decodes).then(() => {}), fallback]).then(() => {
      ...
      if (!startAtShoot) {
        ids.push(window.setTimeout(() => setPhase(1), CUTS[1]));
        ids.push(window.setTimeout(() => setPhase(2), CUTS[2]));
  ...
  return (
    ...
    >
      ...
      {!ready ? null : phase < 3 ? (
        // ── アリーナ3アングル(現在の1枚だけを描画=ハードカット)。key=phaseで切替毎にズーム最初から。 ──
        <div key={phase} style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div
            style={{
              position: 'relative', width: '100%',
              transformOrigin: `${SHOTS[phase].ox}% ${SHOTS[phase].oy}%`,
              animation: `opzoom${phase} ${SHOT_DUR[phase]}ms linear both`,
            }}
          >
            <div style={{ position: 'relative', width: '100%', aspectRatio: `${ARENA_AR}` }}>
              <img src={SHOTS[phase].bg} alt="" draggable={false} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
              {/* キャラ層。flip指定のアングルは左右ミラー(向きを反転)。 */}
              <div style={{ position: 'absolute', inset: 0, transform: SHOTS[phase].flip ? 'scaleX(-1)' : undefined, transformOrigin: '50% 50%' }}>
                {SHOTS[phase].chars.map((c, ci) => (
                  <img
