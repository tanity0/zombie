...
// 撃つ側(社長指定v0.25.2010→2018): 立ち1秒→構え1秒→撃つ0.1→**次のコマ(銃が上=f5)まで行ってから静止**→次0.2→最後(硝煙)は保持。
// 撃たれ側(社長指定v0.25.2011→2012→2014→2016): 被弾=撃つと同期→(静止)→次0.4→次0.4→最後(倒れ伏す)は保持。
// 【静止】撃たれた瞬間=赤くなった瞬間(2.0s)から4.0sまで静止(社長指示v0.25.2017)。撃つ側のみ発砲コマを
// 0.1秒見せてf5(銃上向き)に進んでから止まる(社長指示v0.25.2018)。静止後に各自のテンポで再開。
const SHOOTER_TRACK = [{ t: 0, f: 1 }, { t: 1000, f: 3 }, { t: 2000, f: 4 }, { t: 2100, f: 5 }, { t: 4000, f: 2 }, { t: 4200, f: 6 }];
const VICTIM_TRACK = [{ t: 0, f: 1 }, { t: 2000, f: 2 }, { t: 4000, f: 3 }, { t: 4400, f: 4 }, { t: 4800, f: 5 }];
const RED_FROM = 2000; // 撃った瞬間から背景を赤一色に(社長指示v0.25.2013。以降ずっと赤のまま暗転へ)
...
const SHOOT_FADE_START = 6100; // 最終コマを約1.3秒見せてから暗転(保持長は従来踏襲の叩き台)
const SHOOT_FADE_MS = 1200;
const SHOOT_TOTAL = 7500;
