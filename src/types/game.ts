...
export interface Player extends DashLocomotionState {
  ...
  // ★被弾リアクション(社長指示2026-09-16「食らったらノック、怯み、しゃがみ、ストップ」)の
  // **描画専用**打刻。判定・挙動には一切使わない。時計は Date.now 基準。
  // これが立っている間、本体の絵を**しゃがみ(`-ready`)へ差し替える**(変形はしない=社長指示
  // 「歪ませなくていいんじゃない? しゃがみの絵があるから」)。
  lastHurtAt?: number;
  // 被弾の重さの段(0=軽 / 1=中 / 2=重)。しゃがみの尺とヒットストップの長さを引くのに使う
  // (表は `utils/playerHurt.ts`)。**敵の体勢値とは別系統**——プレイヤーは無敵1秒が頻度を守るので、
  // 「出す/出さない」ではなく「どれだけ効いたか」を段で出す(社長裁定2026-09-16)。
  lastHurtTier?: 0 | 1 | 2;
...
export interface Enemy {
  ...
  lastHit: number;
  // ★直近の被弾で入ったダメージ量(**描画専用**・`lastHit` と同じ瞬間に書かれる・v0.25.4376)。
  // 描画側が**のけぞりの強さ**をこれで決める(`utils/hitFlinch.ts`。社長裁定2026-09-16「0.15から」)。
  // 旧v0.25.4374はチャネル(`'dot'`)で丸ごと除外していたが、量で決める形へ置き換えた。
  ...
  aiPhase?: 'windup' | 'charge' | 'crouch' | 'jump' | 'recover' | 'zpause' | 'zrush' | 'scream'
    ...
    // 立つ位置)はここ(biteAtより前)。
    // ★z-recover(§16-3z「歯応え」の仕上げ): 2発目解決後の硬直。その場で伸び切ったまま
    // 下がらない(=プレイヤーの取り分)。技の続きとして扱う(chaffMoveは立てたまま・s-recoverと同型)。
    // ★z-retreat(§16-A 7条目「技を出し切ったら得意な距離へ離れる」・社長指示2026-09-17):
    // 硬直(z-recover)明け→帯の真ん中(150px)まで後退。s-retreatと同じ型(硬直が先・後退が後)。
    // 紫の追尾(zrush)が終わった後もここへ入る(こちらは技の続きではないのでchaffMoveは使わない)。
...
export type SubWeaponKey = 'heavy-grenade' | 'marksman-trap' | 'striker-quick-mag' | 'striker-hunting' | 'dog' | 'katana' | 'murasame' | 'decoy' | 'shield' | 'whip' | 'alchemy' | 'turret' | 'shijin' | 'fire-knife' | 'drone-boomerang' | 'wire-anchor' | 'sage-stone' | 'homing' | 'shadow-clone' | 'molotov' | 'first-aid-kit' | 'sensor-mine' | 'support-sniper' | 'flare-gun' | 'junk-weapon' | 'gold-ring';
