...
// v65: 着弾の火(fx/hitfire-0/1)を社長支給のドット版シート(_72)から切り出して同名差し替え
//      (0=破裂132×128 / 1=細い噴射120×52。どちらも根元=左・+x噴きで旧素材と同じ向き)。
// v66: 銃口フラッシュを社長指定の正版_70(白・172×128・旧素材と相似=無加工)へ差し替え直し
//      (v63で入れた_66は誤当てはめ=用途不明に戻す)。地割れ(fx/ground-crack)も_68(508×512)で同名差し替え。
// v67: 種弾(fx/plant-seed)を_65(赤いイガ状の球・136×136へ内容クロップ)で同名差し替え。
// v68: ↑を撤回(社長「たぶんその弾じゃない」=誤当てはめ)。旧素材(茶色の種・83×66)へ復元。
// v69: 裏ボス「ミーミル」を社長支給のドット版(804×1024・巨大眼+群体)で同名差し替え。
// v70: 偶像の拳(fx/idol-fist)を社長支給のドット版(400×512・ナックル下端=旧素材と同向き)で同名差し替え。
// v71: 裏ボス「トール」を社長支給のドット版(1024×960・鬼刀の武人+茨の翼)で同名差し替え。
// (v0.25.2943のプラント復元はバンプ無し: 同名差し替えは内容ハッシュ(?v=)が自動で変わるため
//  本定数を上げる必要が無い——上記ヘッダの掟どおり。v69〜71の各バンプも本来は不要だった。)
