...
// サークルが累積ドリフトするので、必要なら実ファイル長から逆算した値をここに焼く(?intN より弱い・BPM算出より優先)。
// Lv1(dance-100.mp3)は v0.25.416 で音源自体を 600ms/拍(=100BPM ちょうど)へリタイム済み → 焼き込み不要(0=BPM算出で600)。
// Lv2/Lv3 の曲も公称とズレていれば、同様に音源リタイム or ここへ実測値を焼く。
export const RHYTHM_INTERVAL_MS_BY_LEVEL = [0, 0, 0, 0]; // idx0=フォールバック / 1=Lv1 / 2=Lv2 / 3=Lv3
...
export const RHYTHM_RESYNC_MS = 1000;         // 位相リシンクの間隔(ms)。1秒に1回(currentTime読み＋必要時のみset)で負荷ほぼ0
export const RHYTHM_RESYNC_MIN_MS = 3;        // この閾値(±ms)を超えたズレだけ補正。タイトに追従しつつ微振動は防止
