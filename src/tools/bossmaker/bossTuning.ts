...
export interface BossTuningEntry {
  ...
  /**
   * このボスが**フェーズ切替(P2)を持つか**(未定義=持たない)。
   * ★v0.25.3563(社長報告「ボスメーカーの上に並んでるメニュー群が効いてない」): 上端の `P2` は
   * `bossPhase: 2` を書くだけのボタンで、**フェーズを bossPhase で持つボスにしか意味が無い**
   * (賞金首では押しても何も起きない=「効かないボタンが並んでいる」の実体)。
   * ボス側が宣言し、UIは**持たないボスではボタンごと出さない**(§2-3「UIはボスを知らない」を守る)。
   * ※舞妓(bounty-maiko)の型B切替はHP閾値方式なので、`HP40%` ボタンで実質到達できる=P2は出さない。
   */
  hasPhase2?: boolean;
  /** 再生の実行(ボス側の状態機械へ繋ぐ)。 */
