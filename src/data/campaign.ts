...
export const SKILLS: Record<SkillKey, { name: string; desc: string; rarity: SkillRarity }> = {
  ...
  'knife-master': { name: 'ナイフマスター', desc: '近接の一撃もコンボに数える。コンボが続くほど全攻撃が重くなり、近接のクリティカルが出やすい。代わりに弾薬は落ちなくなる', rarity: 'rare' },
...
const SKILL_LEVEL_INFO: Partial<Record<SkillKey, { base: string; lv?: [string, string, string] }>> = {
  ...
  'knife-master': { base: '近接の一撃もコンボに数える＋コンボでダメージ増加＋近接のクリティカルが出やすい。ただし弾薬は落ちない', lv: ['+2%/hit(上限+40%)・近接クリ+10%', '+2%/hit(上限+50%)・+15%', '+4%/hit(上限+60%)・+20%・覚醒=近接の間合いが常にハンティング相当'] },
