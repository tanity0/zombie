...
export const SKILLS: Record<SkillKey, { name: string; desc: string; rarity: SkillRarity }> = {
  ...
  'knife-master': { name: 'ナイフマスター', desc: '通常攻撃(近接ヒット)もコンボに加算(窓3秒)。コンボでダメージ増加(+2%/hit, 最大+60%)。近接クリ率+20%。ただし弾薬ドロップ0%', rarity: 'rare' },
...
const SKILL_LEVEL_INFO: Partial<Record<SkillKey, { base: string; lv?: [string, string, string] }>> = {
  ...
  'knife-master': { base: '近接ヒットがコンボに加算(窓3秒)＋コンボでダメージ増加＋近接クリ率上昇。ただし弾薬ドロップ0%', lv: ['+2%/hit(上限+40%)・近接クリ+10%', '+2%/hit(上限+50%)・+15%', '+4%/hit(上限+60%)・+20%・覚醒=近接の間合いが常にハンティング相当'] },
