...
export const SKILLS: Record<SkillKey, { name: string; desc: string; rarity: SkillRarity }> = {
  ...
  'fire-shooter': { name: 'ファイアシューター', desc: '撃った弾のいくらかが爆発弾になる。続けては起きない', rarity: 'rare' },
  ...
  'knife-master': { name: 'ナイフマスター', desc: '近接の一撃もコンボに数える。コンボが続くほど全攻撃が重くなり、近接は急所に入りやすい。代わりに弾薬は落ちなくなる', rarity: 'rare' },
  ...
  'reflex':       { name: '反射神経',       desc: '被弾すると反撃の爆発。続けては出ない', rarity: 'normal' },
...
const SKILL_LEVEL_INFO: Partial<Record<SkillKey, { base: string; lv?: [string, string, string] }>> = {
  ...
  'fire-shooter': { base: '発射が一定確率で爆発弾になる。続けては起きない', lv: ['20%', '25%', '30%・覚醒=大爆発(範囲×1.8)'] },
  ...
  'knife-master': { base: '近接の一撃もコンボに数える＋コンボでダメージ増加＋近接の急所率上昇。ただし弾薬は落ちない', lv: ['+2%/hit(上限+40%)・近接クリ+10%', '+2%/hit(上限+50%)・+15%', '+4%/hit(上限+60%)・+20%・覚醒=近接の間合いが常にハンティング相当'] },
  'benkei':       { base: '武器を切り替えるとクリティカル率が一時上昇。切替先の弾倉が0でも1発装填', lv: ['+5%/10秒', '+10%/12秒', '+15%/15秒・覚醒=2発装填'] },
  'reflex':       { base: '被弾すると反撃の爆発。続けては出ない', lv: ['ダメージ60／半径92／間隔1.0s', '80／104／0.8s', '100／116／0.6s'] },
  ...
  'time-keeper':  { base: 'サブウェポンのクールダウン減少', lv: ['-10%', '-20%', '-30%・覚醒=近接(カウンター)の再使用も-10%'] },
  'ghost-shooter':{ base: '一定確率で弾を消費しない', lv: ['10%', '20%', '30%・覚醒=消費しない代わりにリザーブから即補填(弾倉が減らない)'] },
  'dog-run':      { base: '犬のクールダウン短縮（犬装備時）', lv: ['再使用-50%', '待ち時間なし', '待ち時間なし＋射程制限解除'] },
  ...
  'incendiary-round':{ base: '被弾させた敵を燃焼させる(秒間ダメージ)', lv: ['毎秒2を3秒', '毎秒2を3秒＋着弾地点に小さな炎床', '毎秒4を4秒＋炎床(大)・覚醒=延焼が敵同士の接触で移る'] },
  ...
  'blood-treads':    { base: '移動軌跡に棘を残し、触れた敵に継続ダメージを与える', lv: ['2秒間・毎秒2', '3秒間・毎秒3', '4秒間・毎秒4・覚醒=触れた敵を鈍足化'] },
};
