...
export const SKILLS: Record<SkillKey, { name: string; desc: string; rarity: SkillRarity }> = {
  ...
  // ice-shot: v0.25.3280裁定(ボスも対象・効果半分)に文面を追従。
  'ice-shot':        { name: 'アイスショット',       desc: '命中した敵を鈍足化(ボスは効果半分)。キル時に氷片を3個飛ばす(通常弾の0.3倍ダメージ)', rarity: 'normal' },
  ...
  'execution-shock': { name: '処刑',                 desc: '近接フィニッシュ時、その場で爆発を起こす(近接攻撃の表示ダメージ基準・ノックバック付き)', rarity: 'rare' },
...
const SKILL_LEVEL_INFO: Partial<Record<SkillKey, { base: string; lv?: [string, string, string] }>> = {
  ...
  'bomber':       { base: '手榴弾の爆発時にミニ手榴弾を撒く', lv: ['ミニ3個', 'ミニ3個', 'ミニ3個・覚醒=4個'] },
  ...
  // ice-shot: v0.25.3280裁定(40/50/60%×2/2.5/3秒・ボスは効果半分)に文面を追従(旧文は数値が嘘になっていた)。
  'ice-shot':        { base: '命中した敵を鈍足化(ボスは効果半分)。キル時に氷片3個(通常弾の0.3倍)を飛ばす', lv: ['鈍足40%・2秒', '鈍足50%・2.5秒', '鈍足60%・3秒'] },
  'vampire':         { base: 'キル時、一定確率でHPを回復(発動率は固定)', lv: ['HP+2', 'HP+4', 'HP+6・覚醒=近接ヒットでも最大HPの1%回復'] },
  'incendiary-round':{ base: '被弾させた敵を燃焼させる(秒間ダメージ)', lv: ['秒2×3秒', '秒2×3秒＋着弾地点に小さな炎床', '秒4×4秒＋炎床(大)・覚醒=延焼が敵同士の接触で移る'] },
  'execution-shock': { base: '近接フィニッシュ時、その場で爆発を起こす(近接攻撃の表示ダメージ基準・ノックバック付き)', lv: ['半径80・近接30%', '半径100・40%', '半径120・50%・覚醒=巻き込んだ敵を強くノックバック'] },
  'gravity-shot':    { base: 'キル時、一定確率で周囲の敵を中心へ引き寄せる爆縮を発生させる', lv: ['発動20%・半径100', '発動30%・半径120', '発動40%・半径140・覚醒=2倍の長さで引き寄せ続ける'] },
