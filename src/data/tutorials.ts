...
export const TUTORIALS: TutorialEntry[] = [
  ...
  },
  {
    ...
    // 手本GIF(実機収録・社長撮影v0.25.2254/2255)。**2種類を1本に繋いである**:
    //   前半 = 通常射撃(立ち止まって撃つ→弾が飛ぶ→120ダメージ)
    //   後半 = 吸い付き(狙いサークルが敵の頭に緑で乗る)
  ...
  },
  {
    ...
    // 手本GIF(実機収録・社長撮影v0.25.2259): **見つかる → 下がる → 壁の裏(下)に隠れる →
    // 薄い赤の視界が壁の上に取り残される**まで。「壁で視線を切れば見つからない」がそのまま見える。
    img: 'tutorial/m2-scout.gif',
    where: '研究所',
  },
];
