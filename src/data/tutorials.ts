...
export const TUTORIALS: TutorialEntry[] = [
  ...
  // 手本はステージ1(森)でヘッドレス収録したもの。洞窟で撮り直すかは社長裁定(★未決)。
  {
    ...
    // 手本=**実機収録**(社長撮影v0.25.2306)。近づいてくる相手を自動射撃で倒し、**弾数が減って0になる**
    // ところまで映っている(=「弾はちょうど倒せる分だけ」という台本がそのまま画になっている)。
    // 尺は切らずそのまま。旧 `action-shoot.mp4`(ヘッドレス収録・森)は素材として残置。
    img: 'tutorial/m0-shoot.mp4',
    where: '訓練',
  },
  {
    id: 'm0-melee',
    ...
    // 旧 `action-melee.mp4`(ヘッドレス収録・森)は素材として残置(v0.25.2330で `m0-crit` も実機収録へ
    // 差し替えたため、現在どのチュートリアルからも参照されていない)。
  ...
  },
  {
    ...
    // 社長の実機収録へ差し替え(v0.25.2330)。旧 `action-melee.mp4`(ヘッドレス収録・森)は
    // 近接の手本の流用で、クリティカル固有の見せ場(金の数字・体勢崩しの黄輪)が入っていなかった。
    // 決まる瞬間は setpts で2.6倍スローにしてある(社長指示v0.25.2266の流儀)。
    img: 'tutorial/m0-crit.mp4',
    where: '訓練',
  },
  {
    id: 'm0-finish',
  ...
  },
  {
    id: 'm0-counter',
    title: 'カウンター',
    // **弾き返し専用**の説明にする。気絶/一撃は `m0-crit` / `m0-finish` の担当で、ここに混ぜない
    // (社長指摘v0.25.2299「近接フィニッシュとカウンターがごっちゃになってる」)。
    lines: [
      '飛んでくる弾に合わせて近接を振ると、弾き返せる。',
      '弾き返した弾は、撃った本人を撃ち抜く。',
      'タイミングは短い。弾をよく見ろ。'
    ],
    // 手本=**実機収録**(社長撮影v0.25.2304)。「敵の弾が飛来 → Counter! → 弾き返す」が一続きで映る。
    // 尺は切らずそのまま。旧 `action-counter.mp4` は近接フィニッシュ側の手本。
    img: 'tutorial/m0-counter.mp4',
    where: '訓練',
  },
  {
    id: 'm0-levelup',
  ...
  },
  {
    ...
    // 手本=**実機収録**(社長撮影v0.25.2303)。ヘッドレス収録(`demo-area-wall.mp4`)では
    // 銘打ち(大きな「研究対象区域 —— 踏破」)がどうしても写らず、白い境界線だけの絵になっていた
    // (社長報告v0.25.2302・スクショ付き)。実機なら儀式がそのまま映る。**尺は切らずそのまま**。
    img: 'tutorial/m0-area.mp4',
    where: '訓練',
  },
  {
    id: 'm0-hunter',
  ...
  },
  {
    ...
    lines: [
      '拠点を確保すると、その方角にある建物の場所がわかる。画面の端に出る矢印がそれだ。',
      '緑の十字は廃病院。サークルに入って待てばワクチンが手に入る——倒れても一度だけ立ち上がれる。',
      '琥珀の木箱は武器庫。スクラップを払えば、軍用装備と交換できる。',
      '青の星は警察署。近づけば囲まれる。全滅させれば、そこでしか手に入らない力をこの出撃の間だけ使える。',
      '赤いドクロは寄り道ではない。深いところに巣くう個体だ。',
      '寄り道は自由だ。ただし往復すれば、そのぶん時間を失う。',
    ],
    // 手本メディアは無し(後日社長が実機収録を用意する)。テキストだけで成立する構成にしてある。
    where: '狂い咲きの森',
  },
];
