...
export const TUTORIALS: TutorialEntry[] = [
  ...
  },
  {
    ...
    // 手本GIF(実機収録・社長撮影)。**映像の最初から5秒まるごと**(社長指示v0.25.2261)。
    // 流れ: 敵の視界(薄い赤)の中を進む → 見つかる → 下がる → 壁の裏(下)に隠れる →
    // 薄い赤の視界が壁の上に取り残される。「壁で視線を切れば見つからない」がそのまま見える。
    // 尺が長いのでコマ数を抑えるため 7fps・360x225・64色(赤の視界が飛んでいないことは検証済み)。
