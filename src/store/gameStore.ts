...
// v0.25.3138(社長指示「グレンの触手の予兆を少しだけ延長」): 実効300ms → **420ms**。
// v0.25.3142(社長指示「触手、溜め時間もう少し長くして」): さらに **420ms → 540ms**。
// 3連発の周期(実効1.0秒)は動かさないので、伸ばした120msは**間(GAP)から引く**
// =「1秒置きに撃つ」というリズムは1msも変えずに、読む時間だけ増える。
export const GLEN_REACH_WINDUP_MS = 648;            // 実効540ms・静止
...
// 1周期を**きっちり実効1.0秒**に割る: 溜め540ms(予告) + 実行220ms + 間240ms = 1000ms。
// (v3126時点は 300/220/480 → v3138で 420/220/360 → v3142で 540/220/240。予告を伸ばすたびに
//  同じ量を間から引いている=「1秒置き」のリズムは3版を通して1msも変わっていない。)
// 予告を伸ばす時は必ず**間から引く**=「1秒置き」のリズムを壊さない。伸ばした尺を実行側へ置くと=
// 間は硬直と同じ扱い(赤い帯を出さない)ので「赤いのに当たらない」を作らない。
// ※これ以上予告を伸ばすなら間(残り240ms)では足りない。**周期そのものを伸ばす=社長判断**になる。
export const GLEN_REACH_SHOTS = 3;                  // 何発撃つか(固定3=数えられる)
export const GLEN_REACH_GAP_MS = 288;               // 実効240ms(= 1000 − 溜め540 − 実行220)
