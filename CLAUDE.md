...
- **★★赤い予告の2つの掟(社長指示2026-09-18・技を足すたびに監査が止める)**:
  社長の言葉「**この赤いルールは全部流星であることと、始まりと終わりのタイミングについては、
  技が追加されるたびにちゃんと止めるように**」。
  ...
  2. **★消え切る時刻 = 当たる時刻。**1msでもズレたら予告は嘘になる。
...
「**★★赤い予告の2つの掟**(流星であること/消え切る時刻=当たる時刻。**技を足した回は必ず止める**)」
...
- **体制の更新(社長決定v0.25.1705): Sonnet単独チャットは廃止。「Sonnet」=設計チャットから起動する
  ...
  - **実装(Sonnetサブエージェント)**: 設計書のバッチを実装し、結果を DEVELOPMENT_LOG.md に残す。
    ...
    「★★赤い予告の2つの掟」(流星であること/消え切る時刻=当たる時刻・多段は段ごと)を発注文へ写す**
    (社長指示2026-09-18)。
