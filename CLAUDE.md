...
- **★★敵の絵は「変換スタジオ」の出力(2026-09-23 判明)。ドット数は描き直しではなく“つまみ”で決まる。**
  社長が支給する敵シートは**ツールの変換出力**で、**解像度(高さ)の設定値がそのままドット数**になる
  (城ボス1の跳びは **130** で出し直した実績=v0.25.4576)。**ドット絵師に発注し直す工程は存在しない。**
  - **色数は1バイトもメモリに効かない**(実測: 同じシートを2色まで減色してもメモリは3.80MBのまま。
    **半分の寸法にした時だけ 0.95MB になる**)。効くのは**縦×横の画素数だけ**。
  - **目標は「絵のドット数 ≒ 画面での表示px」。** レンダラの解像度(PC=2/スマホ=1+DPR)が勝手に
    2〜3倍へ拡大するので、**1:1にすると1ドットが2〜3実画素=HD-2Dの粗さ**になる。
    表示px(縦)の目安: リッチ85 / 伐採人115 / 城ボス1 140 / ハンター180。
  - **★★ボスゾーンは高さ256基準・雑魚ゾーンは128基準(社長決定2026-09-24「**今後、ボスゾーンは
    256基準になると思う**」)**。実測の表示px(縦)は**ボス帯=ミーミル372 / 使者276 / ヨルムンガンド270 /
    トール210 / スカディ204 / 死神174 / ハンター170**、**雑魚帯=伐採人118 / ゾンビ113 / 削岩型107 /
    叫喚型105 / 骸骨100 / コウモリ92 / 蜘蛛88**で、**ボス帯は雑魚帯のちょうど2倍**。
    だから「ドット数≒表示px」に当てると **256 / 128** の2段になる。※**ミーミルだけは256でも足りない**(372)。
    - **代償を先に言う**: メモリは**縦×横×4バイト**なので**高さが倍=1コマ4倍**。16コマのシート1枚が
      **約1MB→約3.3MB**(死神の実測 3424×256=3.34MB)。ボス帯7型に歩き+攻撃を入れると **+40MB前後**。
    - ★**シートも遅延組へ回した(社長指示2026-09-24「乗せて」・v0.25.4624 で実施済み)**。
      かつては `pixiTextures` の起動マニフェストが7つのシート表のキーを**そのまま全部展開**していたため、
      **表に1行足すだけで全ステージぶんが起動時常駐**になっていた。いまは
      **`utils/enemySheets.ts` の `SHEET_RESIDENCY`(立ち絵名→`deferred`/`eager`)が1本の並びを2つに割る**。
      - **決め方は「登場に猶予があるか」と「そのステージにしか出ないか」**。カットイン/アテンションを
        挟んで出て、かつステージが限られる個体は `deferred`。**前触れなくその辺に居る敵**(雑魚・強個体)と
        **全ステージで出る個体**(ハンター=遅延にしても常駐が減らない・M0は予告なしで画面に出る)は `eager`。
      - **姿が見えた時点でその個体のシートを全部取りに行く**(`warmEnemySheets`)。網(`getTexture`)だけだと
        **跳び/薙ぎ/攻撃のシートは「最初にその技へ入った瞬間」まで1度も引かれない**=最初の1発が欠ける。
      - **外れても軽い**: シートが間に合わないと描画は**立ち絵へフォールバック**する(敵は消えない)。
      - **書き忘れ・台帳漏れはテストが止める**(`sheetResidency.test.ts`)。
        ①常駐1.0MB超のシートは決定表に必ず載っていること ②表のキーは実在すること
        ③**全シートが原盤台帳に載っていること**(`assets:check` は新顔を注意として出すだけで落とさないため)。
  - ★**ただし遠近スケール(`ENEMY_DEPTH_MIN..MAX`=0.55〜1.85)が掛かる**ので、1:1だと
    **敵が画面の上(奥)に居る時はドットが落ちる**。落とさせたくないなら**表示pxの半分**で作る
    (城ボス1なら70)。**どちらを取るかは社長が実機で決める。**
  - ★★**「ドット/px」の数字だけで決めてはいけない(2026-09-23・実測で判明)。**
    城ボス1の立ち絵を **121×128(0.92ドット/px=1つも落ちない)** と **335×356(2.55ドット/px=6割落ちる)** で
    出し、**実際の描画寸法に落として並べたら 335 の方が明らかに良かった**(121版は減色で中間調が黒へ潰れ、
    暗く・のっぺりしていた)。**落ちるドットの数より、変換時の減色の質の方が効く場面がある。**
    ⇒ 数字は**候補を絞るため**に使い、**採否は実寸で並べた画で決める**(社長裁定: 335を採用)。
- **★`public/` は「原盤」。縮小版で上書きしない(社長指示2026-09-16「原盤確保の仕組み化」)**:
  起動時のテクスチャが **308MB**(2Dゲームの相場は常駐50〜150MB / iOSがタブを落とすのは概ね200〜400MB)あり、
  **端末のメモリ天井で落ちている**。効く手は **①読まない ②寸法を下げる** の2つだけで、メモリは
  **幅×高さ×4バイト**で決まる=**PNGの圧縮率は1バイトも効かない**(例: `fog-alpha.png` はディスク76KB/メモリ6.0MB)。
  ②をやる時、**配信用の縮小版で原盤を上書きすると二度と戻せない**。社長方針は
  「**別のもの(Steam等)として出す前提で元素材は取っておく**」=`public/` は原盤、縮小版は**出力側で作る**。
  - **機械で守ってある**: `npm run assets:check`(CIで毎push)。**寸法が縮んだ/消えた素材があれば落ちる**。
    差し替え(寸法そのまま)は通る。素材を足した/差し替えた時は `npm run assets:ledger` で台帳を更新する。
  - ①の側(読み込みを遅らせる)は `pixiTextures.ts` の `DEFERRED_SPRITE_GROUPS`。
    **出撃時にステージぶんだけ先読み+描く瞬間の網**の2段構え(v0.25.4353/4354)。
...
- **★★赤い予告の4つの掟(社長指示2026-09-18・技を足すたびに監査が止める)**:
  社長の言葉「**この赤いルールは全部流星であることと、始まりと終わりのタイミングについては、
  技が追加されるたびにちゃんと止めるように**」+「**基本的に全部流星にしたい。ルールとしても
  出始めと合わせる。なので、通るものもそれに合わせて**」。
  1. **赤い予告は必ず流星にする。**「外から内へ赤が流れて、**消え切った瞬間に当たる**」。
     溜めが進むほど濃くなるだけ/点滅するだけ/最初から最後まで同じ濃さ、は**流星ではない**=違反。
     位相の出どころは `telegraphStyle.ts` の `meteorPhase` 1本(自前で位相を作らない)。
  2. **★出る時刻 = 溜めが始まる時刻。**(社長指示2026-09-18「**出始めと合わせる**」)
     **遅れて出る**のは反応時間を削る嘘、**早く出る**のは「まだ来ないのに赤い」嘘。
     ★**1フレームも出ないのは論外**(実測: フィルの祝福の1発目は、生成時刻が溜め満了と同時のため
     `if (gameTime >= q.at) continue` で**予告が1度も描かれない**)。
  3. **★消え切る時刻 = 当たる時刻。**1msでもズレたら予告は嘘になる。
     ★**多段の技は段ごとに別々の時刻を持つ**——1本の予告で複数の命中をまとめない。
     **実測の前例(v0.25.4454)**: バス停の三段突きは**3本の帯が溜め明けに同時に消える**のに、
     実際に当たるのは **+90ms / +310ms / +530ms**(各段の周期が 220/220/150ms で、突き出し90msの末尾に
     判定が積まれるため)。**最大0.5秒の嘘**で、「赤が消えた=終わった」と読んだプレイヤーが3発目を食らう。
  4. **★「通る」技(突進・薙ぎ抜け)は、当たり得る間ずっと赤が在る。**(社長指示2026-09-18
     「**通るものもそれに合わせて**」) 突進は**接触で当たる**ので当たる瞬間が1点に定まらない。
     **赤が消えている間は当たらない**が全型で成り立つこと。**走り出しで赤が消える実装は違反。**
     - ★**「走行中に体幅ぶんの赤帯を出す」は却下された見せ方**(社長指示2026-09-18「自転車に流星の
       あと紅いライン引いたね？これやめてほしいんだけど、ほかの敵や技にも引いた？全部取っ払って」
       →「**トールも予告の流星入ってるよね？であれば消して**」)。v0.25.4458 で5州へ広げ、
       v0.25.4474 でその5州を、**v0.25.4475 で先例だったトールの分も撤去**(コードごと削除)。
       **理由: 突進には既に溜めの流星ラインが出ているので、走行中の帯は「流星のあとに残る紅いライン」
       に見える。** **掟は残るが、体帯は出さない。**再検討する時は**別の見せ方**から始めること。
  - ★**①の唯一の例外(社長裁定2026-09-18)**: **ミーミル/バス停のレーザー(紫)の「カラオケ塗り」**
    (根元→プレイヤー足元へ伸びる独自の読み方)は**残す**。**①だけ外れる**のであって、
    ②(出る=溜め開始)③(消え切る=当たる)は**レーザーにも適用する**。
    ※ここに書かれていない例外は無い。**監査はこの1件以外を全部違反として挙げてよい。**
  - **いつ止めるか**: **技(新しい `aiPhase` / `bossState` / 新しい命中の積み方)を足した時は必ず**。
    数値の微調整だけなら不要。★**「予告が無い攻撃を足した」も違反**(赤が無いのに当たる)。
  - **監査へ渡す時はこの2行をそのまま渡す**(品質監査・クリエイティブ監査のどちらでも)。
- **★★元々あるエフェクトを、指示なく消さない(社長指示2026-09-21・MUST)**: 社長の言葉
  「**元々のエフェクトは消さないでね**」「**基本的にこちらから指定しなければ元々のエフェクトは消さないで**」。
  - **新しい絵(アニメーションシート等)を入れる時も、既にあるエフェクトはそのまま出す。**
    「シートに描かれているから重複する」と**自分で判断して消さない**——実際、骸骨の爪を
    「二本に見える」と判断して止め、**社長裁定で差し戻した**(v0.25.4543〜4547)。
    絵とエフェクトが重なって困るかどうかは**実機で社長が見て決める**。
  - **消したい理由があるなら、消さずに提案する**(①何が重なるか ②消した場合/残した場合 ③推薦)。
  - ★★**モーションを足して外してよいのは「歪み」だけ**(社長指示2026-09-22
    「**モーション追加により外すのは歪みだけです**」)。アニメーションシートを入れた時に止めるのは、
    **スプライトそのものを伸び縮み・回転させる代用モーション**だけ:
    **①歩行の傾ぎとスカッシュ ②疑似呼吸 ③技のスカッシュ(しゃがみ/滞空/着地)**。これ以外は**何も外さない**。
    - **外さないもの(全部残す)**: エフェクト(砂埃・血・リング・VFX)/ **別スプライトの武器**(爪・ランタン)/
      **位置の移動**(跳ぶ高さ・上下の揺れ・路面振動・震え)/ SE / 光。
    - ★**「シートに描いてあるから重複する」は外す理由にならない。** 重なって困るかは実機で社長が決める
      (上の「元々あるエフェクトを、指示なく消さない」と同じ。武器を止める表は**空が正**)。
    - 判定に迷ったら: **その処理はスプライトを伸ばす/縮める/回すか?** → はいなら歪み(外す)、
      いいえなら残す。
...
- **毎pushで `src/data/changelog.ts`(タイトル画面の更新情報)にも先頭へ1エントリ追記する
  ...
  - **★書き方(クリエイティブ監査2026-09-11 #6「更新情報が開発日誌」の是正・以後の規約)**: 更新情報は
    **プレイヤーが画面で見る言葉で書く**。①**★・矢印・絵文字を使わない** ②**内部語を使わない**(流星/城ボス/
    設計書/監査/バッチ/DEVLOG/変数名。「城ボス」は「ステージ◯のボス」、「流星化」は「赤い予告が外から内へ流れる」)
    ③**内部の調整値を書かない**(px・ms・%・倍率・版数。「310→400」ではなく「棘が長くなった」。ステージ番号・
    回数の漢数字は可) ④**「〜になりました」を3項目続けない**(言い切り・体言止め・「〜が直った」を混ぜる)
    ⑤文書のみの回は定型の1行だけ(説明を足さない) ⑥**武器・敵・場所の名は data の表記どおり**(例: 「PHILLガン」。
    全角英字・別表記を作らない)。
...
### ★★「AIっぽさ」を排除する責任者=クリエイティブ監査(社長指示2026-09-11・以後の必須手順)
社長の言葉: **「今後ゲームのクリエイティブに関して UI UX とか目に見える部分に関しては必ずその
AI ぽさを排除する責任者である監査を通すように」**。
上の品質監査(ゴール達成/漏れ)とは**別人・別の問い**。ゴールに届いていても、**AIが作ったと分かる
見た目・言葉・動き**は出荷しない。

**対象(=「目に見える部分」)**: プレイヤーの目・耳に届くもの全部——UI/HUD/メニュー/画面遷移・
演出(予告・エフェクト・技のモーション・カメラ)・**文言**(UIテキスト・台詞・changelog の項目・
チュートリアル本文)・配色・レイアウト・SE/BGMの当て方。
**対象外**: 純ロジック・バランス数値・計測/走査・裏方の配線・文書のみの変更。

**いつ通すか**: 対象の変更を**pushする前**(検収監査と同じ場面)。**着手前(設計時)に通してもよい**
(設計の時点でAIっぽい選択を止められるなら、その方が安い)。
- **必須**: 見え方が**新しくなる**変更——新規の演出/UI、作り直し、文言の新規・書き換え。
- **付けない**: 既存の見え方の**数値を動かすだけ**(社長が実機で「もっと濃く」と言った値の追従など)。
  物差しは「★付けない仕事」と同じ=**新しい見え方を足しているか**。判断に迷ったら**付ける**。

**監査役への問い(毎回同じ・1問)**:
> **この作品を人間の作り手が作っていたら、ここはこうしなかった——と思う箇所をすべて挙げよ。**

**渡すもの**: ①成果物そのもの(コード差分+絵が出る条件。スクショが撮れる環境なら必ず撮って渡す)
②**作品の文脈**(HD-2D・ゾンビ・この場面のトーン。**★手本(社長指示2026-09-12): UI/UX は全体的に
デス・ストランディング2をモデルにする。ダークファンタジーホラーの要素があるので、そこは FF とディアブロを少しだけ混ぜる**——
この2行は毎回そのまま渡す。CLAUDE.md の「攻撃ヴィジュアルの2分類」「色と形の文法」
「**★★赤い予告の4つの掟**(流星/出る=溜め開始/消え切る=当たる/通る技は当たり得る間ずっと赤。
**技を足した回は必ず止める**)」
「慣性MUST」は**作品側の掟**なので渡す) ③「ではない」条件(例: 「判定の位置は変えない」「素材は既存のみ」)。
**渡さないもの**: 過程・弁明(品質監査と同じ)。

**監査役に示す「AIっぽさ」の当たり(限定しない・ヒント)**:
1. **均質**——全部同じ強さ・同じ尺・同じイージング・同じ角丸。人は選んで差をつける。
2. **説明過剰**——UI文言が丁寧・長い・全部言う。作品の口調がない。
3. **汎用テンプレ**——どのゲームにもある見た目。この世界でこの色・この光である理由がない。
4. **記号癖**——★・絵文字・矢印・箇条書き的な並び・「〜しました」の連打。
5. **数式的な動き**——全部 smoothstep・完全対称・機械的に等間隔。
6. **無難**——迷って中庸を取った跡(薄い塗り・小さい動き・全部0.5)。
7. **整いすぎ**——手癖・偶然・崩しがない。

**掟**:
- **モデルは Fable 5.1 固定**(品質監査と同じ理由=設計者と同じ頭にしない)。**`model: "fable"` を明示。**
- **褒めない。指摘だけ。**「AIっぽくない点」の列挙は不要。
- **指摘の扱い**: 演出の見せ方・文言・数値の範囲なら**直す**(責任者の権限)。
  **仕様・意匠の意図に触れる**(判定の形・技の役割・世界観の設定)なら**社長へ戻す**(既存ルールどおり)。
- **巡の止め方は検収と同じ判断(★2026-09-16更新)**: 旧「1巡で終える」という固定の打ち切りは**廃止**。
  「★検収の終わり方」3項と同じ材料で判断する(**前の巡で直すべき指摘が出たか / 新しい角度が残っているか /
  (A)以外で巡を増やさない**)。「AIっぽさ」が探せば無限に出るのは事実だが、**それは(A)以外で巡を
  増やさない、で止める**のであって、回数で切ると本物を落とす。
  **実測2026-09-16**: §16のクリエイティブ監査は**2巡目に「体が光る、が実は乗算tintなので沈む」**という
  出荷前提を覆す指摘を出した(このプロジェクトが過去に踏んだ黒tint事故と同型)。**1巡で切っていたら失っていた。**
- 通した結果(指摘数/直した数/社長へ戻した数)を **DEVELOPMENT_LOG に1行**残す。
- **サブエージェント(Sonnet)に実装させた見た目も対象**。実装者ではなく**設計チャットがこの監査を回す**。
...
3. **★「2巡で止める」は廃止(社長指示2026-09-16「悪い方向に働くことがある」)。**
   **代わりに、2巡目からは「次の巡を回す必要があるか」を判断してから回す。**
   固定の打ち切りは、**まだ本物のバグが出ている巡を切ってしまう**ことがある
   (実測2026-09-16: §16の検収で**2巡目に(A)が3件**——しかも全て設計者自身の記述誤り——出たが、
   旧ルールなら「2巡したから実機へ」で止めていた)。
   **判断の材料は3つ**:
   - **①前の巡で(A)が出たか。** 出たなら回す(バグを出す側の巡がまだ生きている)。
     **(A)がゼロなら止めて実機へ**——「★実在確認の掟」どおり**実機で見えているかが最終の真実**で、
     コードの中だけで完璧を目指すより先に社長へ見せる方が速い。
   - **②新しい角度が残っているか。** 同じ問いを繰り返すだけなら止める。違う目
     (例: 「設計の良し悪し」ではなく**「文書内の食い違い/実装者が迷う所」を見る整合監査**)が
     残っているなら、回す価値がある。**角度を変えずに回数だけ増やさない。**
   - **③(A)以外で巡を増やさない。** 網の細かさ・変異への耐性は従来どおり(B)として別案件へ積む
     (これは維持——10巡8時間の再発防止はここが効いている)。
   **回す時も止める時も、判断と理由を1行残す**(DEVELOPMENT_LOG)。「なんとなくもう1回」をしない。
...
- **体制の更新(社長決定v0.25.1705): Sonnet単独チャットは廃止。「Sonnet」=設計チャットから起動する
  ...
  - **実装(Sonnetサブエージェント)**: 設計書のバッチを実装し、結果を DEVELOPMENT_LOG.md に残す。
    ★**技(新しい `aiPhase`/`bossState`/新しい命中の積み方)を足す発注には、必ず
    「★★赤い予告の4つの掟」(流星/出る=溜め開始/消え切る=当たる・多段は段ごと/通る技は当たり得る間ずっと赤)
    を発注文へ写す**
    (社長指示2026-09-18)。
