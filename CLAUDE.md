...
2. **効くと確定しているのは「個数」だけ。「大きさ(半径・塗り面積)」が効くかは未検証。**
   `glowhalo` 1.7→1.3(面積0.63倍)の比較は**壊れた計測器の値しか無い**(v0.25.2679↔2682・n≈4)。
   新計測器で `?glowhalo=` のA/Bを取るまで、**半径を縮めれば軽くなる**という前提で設計しない。
   (`?refl=0` のA/Bでは「加算スプライト2枚ぶんの面積」は測定限界以下だった=面積が効かない側の
   傍証はあるが、強glowの大きな円で確かめたわけではない。)
