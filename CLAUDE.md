...
- **★★敵の絵は「変換スタジオ」の出力(2026-09-23 判明)。ドット数は描き直しではなく“つまみ”で決まる。**
  ...
  - **★★ボスゾーンは高さ256基準・雑魚ゾーンは128基準(社長決定2026-09-24「**今後、ボスゾーンは
    ...
    - ★**いまの配線では、シートは全部「起動時に読む」側へ自動で入る**(`pixiTextures` の起動マニフェストが
      7つのシート表のキーをそのまま展開している)。**ボスの立ち絵は既に遅延組(`DEFERRED_SPRITE_GROUPS`)
      なのに、シートだけ起動時常駐になる**。**256基準で枚数が増える前に、シートも遅延組へ回すかを決める**
      (★裁定待ち)。
