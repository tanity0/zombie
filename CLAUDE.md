...
- **★★元々あるエフェクトを、指示なく消さない(社長指示2026-09-21・MUST)**: 社長の言葉
  ...
  - ★**「エフェクト」と「絵の歪み」は別物**。エフェクト(砂埃・血・リング・VFX・別スプライトの武器)は
    **消さない**。一方、**スプライトそのものを伸び縮み・回転させる代用モーション**は
    「絵が入った敵のパターンには歪み入れないで」(同日の社長指示)に従って**掛けない**
    (歩行の傾ぎ/疑似呼吸/技のスカッシュ)。**位置の移動(跳ぶ高さ・上下の揺れ・震え)は歪みではないので残す。**
