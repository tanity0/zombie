...
- **★敵の仕様は「種類(区分)」で固める(社長指示2026-08-28「敵の仕様は種類で固めたはず。指示するまで
  増やさないで。強個体は強個体です」)**: 敵の扱い(体勢値・KB耐性・HPバー・フィニッシュ免除等)は
  区分(雑魚/強個体/ボス級/終端)のテンプレに従う。**区分に属する敵には区分の仕様が全部付く**
  (例: 強個体=体勢値あり——パンプキンだけでなく削岩型・伐採人も)。逆に、**個別の型に区分外の仕様を
  足す提案・実装は社長の指示があるまでしない**(「この型にも◯◯を付けますか?」と型ごとに聞かない。
  区分との不整合を見つけたら「区分仕様への整合」として直す=それは増設ではない)。
...
**ゴールに求めるクオリティだけを見る監査サブエージェントを1人、下の2場面で必ず付ける。**
評価者は**過程を知らない**。だから「ここまで頑張った」「こういう事情がある」は効かず、
**ゴールに届いているかだけ**が問われる。設計者(=このチャット)の思い込みを外から壊すための装置。
- **★監査役のモデルは Fable 5.1(社長指示2026-09-03)。** サブエージェント起動時に `model: "fable"` を
  明示する(指定を省くと親のモデルを引き継いでしまい、監査が「設計者と同じ頭」になる=外から壊す装置に
  ならない)。**設計チャット側が何で動いていても、監査だけは Fable 5.1 に固定する。**
...
- **★★hook による巻き戻り対策は【失敗と判明】した(2026-08-23・実測)。下の「毎ターンの3点セット」が正。**
  社長指摘「**ほぼ毎回巻き戻ってない?**」を受けて調べたところ、**2026-08-23 の1日で5回**巻き戻っており、
  **戻り先は毎回まったく同じ `0621fc40`(v0.25.3774)=セッション開始時のスナップショット**だった。
  そして **hook は2箇所とも消えていた**(`ls` で確認: `~/.claude/hooks/git-follow.sh` も
  `~/.claude/settings.json` も `No such file`)。理由は**完全な循環**:
  > 巻き戻る → **巻き戻りを直すための hook も一緒に巻き戻って消える** → 次も巻き戻る
  - リポジトリ内の hook は「v0.25.3774 の時点には存在しない」ので**巻き戻ると消える**。
  - リポジトリ外(`~/.claude/`)も**同じ復元で丸ごと消える**。v0.25.3829 の「外なら生き延びる」という
    検証結果は、**その時の1回の巻き戻りでは正しかったが、コンテナ再起動を伴う巻き戻りでは成立しない**。
  - **⇒ どこに置いても hook では直らない。生き残るのは origin だけ。**

- **★★原因が確定した(2026-08-26・監査+サブエージェント3人で究明・社長指示)。「巻き戻り」の正体は
  git でも GitHub でもなく、★このコンテナのディスクが `2026-08-21 15:39:27 UTC`(= 08-22 00:39 JST)で
  凍結していること。** それ以降、このディスクには**1バイトも書かれていない**。毎セッションは
  その凍結イメージの使い捨てコピーとして起動するので、**HEAD は必ず `0621fc40`(v0.25.3774)に戻る**。
  - **物証(3つが独立に一致)**:
    ①`git reflog` が `0621fc40 @2026-08-21 15:32:59` の次に**いきなり 08-26 の reset**へ飛ぶ
    (08-22〜25 の40コミットは全て**別のディスク**で作られ、リモートにだけ在る)。
    ②`/var/log/dpkg.log` と `/tmp/environment-manager-*.diag.log`(414本)のブート痕跡が
    **08-03〜08-21 は毎起動ぶん累積しているのに、08-22・23・24・25 が完全に欠落**している
    (=それ以前はディスクが再起動をまたいで**永続していた**)。
    ③`find / -xdev -newermt "2026-08-21 15:45" ! -newermt "2026-08-26 03:35"` の結果が**外部マウント2件のみ**。
  - **凍結の瞬間はコンテナの正常終了(`exit_code:143`=SIGTERM)の直後**。つまり 08-21 15:39 を境に
    「再起動をまたいで生き残る永続ディスク」→「毎回イメージから復元される使い捨てディスク」に**挙動が変わった**。
    戻り先が毎回まったく同じなのは、**復元元のイメージが1つしかない**から。
  - **★社長の仮説「private化したから」は否定された。** `tanity0/zombie` は**現在も public**(private化は
    裁定だけで未実施)。08-21 のコミットは STATUS と設計書の**文言だけ**で設定変更を含まない。
    `.github/workflows/` は **08-03 以降1度も変更なし**。`.gitignore`/`.gitattributes` も 08-14 が最終。
    **リポジトリ内に巻き戻りを起こしうる変更は1つも無い。変わったのはプラットフォーム側。**
  - **★退避先は存在しない。** `/`・`/root`・`/home/user`・`/tmp`(scratchpad 含む)は**全て同一デバイス**
    (`stat -c %d` が全部 65024)。だから `~/.claude/` も scratchpad も**同時に巻き戻る**。
    **VM の中に置いた対策は原理的に1つも生き残らない。**
  - **★戻り先が定数なので、検知は1行で書ける(新情報・毎ターンの3点セットの前に置ける)**:
    `git rev-parse --short HEAD` が **`0621fc40` なら巻き戻り確定**。
  - **★これは社長側からしか直せない。** 壊れているのは**このコンテナ個体の永続化**であって、
    リポジトリでもエージェントの手順でもない。**セッション/コンテナを作り直せば壊れた個体から離れられる**
    (代償=このセッションの会話コンテキストから切り離される)。判断は社長。

### ★毎ターンの3点セット(巻き戻り対策の正・2026-08-23)
1. **ファイルに触る前の最初のコマンドとして、必ず追従する。**
   `git fetch origin <branch> && git reset --hard origin/<branch>`
   巻き戻っていなければ何も起きない(無害)。**「たぶん大丈夫」で省略しない。**
2. **すべての文字列置換に `assert` を付ける。** 巻き戻ると置換対象の文字列が存在しないので**必ず落ちる**
   =一次検知器になる。**実際、`package.json` の bump だけ assert を付けていなかったために黙って
   素通りし、戻った HEAD の上にコミットして孤立した枝を作った**(push が non-fast-forward で弾かれて発覚)。
3. **1変更ごとに小さく push。** 失うのは最大1変更ぶんに限定される。**push 直後でも巻き戻る**ので、
   「まとめて後で push」は被害を最大化する。

- (以下は v0.25.3829 当時の記録。**hook はもう当てにしない**が、仕組み自体は残してある。)
  ...
  **ローカルが origin より後れている時だけ `--ff-only` で追いつく**。早送りできない時
  ...
  - **★この「残る穴」が実際に本命だった(2026-08-23)**: 当時「`~/.claude/` がコンテナ再起動を生き延びるかは
    未確認」と書いて保留していたが、**生き延びない**。しかも巻き戻り先はリポジトリ内 hook の追加より
    前のコミットなので、**内も外も同時に消える**。上の「毎ターンの3点セット」で運用すること。
