...
2. **【v0.26.8で廃止・社長指示】憲法テスト(`src/utils/constitution.test.ts`)は`claude/direction-shift`
   ブランチでは拘束力を失った。** 旧・横断不変条件(countCap上限/初心者ゾーン不可侵/featuredの自己整合/
   mix全指定/床の許可リスト/床つき演目の主役上限/天井の単調性)は「全条維持」の縛りではなくなり、
   PACING_V2.mdの現行記述(§2エリア台本 等)が優先する。ただし一般原則(横断的な不変条件をテストで
   機械化する/変更前に社長裁定を得る)自体は引き続き有効。新しい横断ルールを設ける時は同様にテスト化する。
...
5. **完了報告の前に自己点検1行**: 「この変更はPACING_V2.mdの現行設計(エリア台本の上限・関所/緩の
   意図)と矛盾しないか」をDEVELOPMENT_LOGの実装結果に明記する(旧・憲法第4条/5条への言及は
   v0.26.8の廃止以降は不要)。
...
- 現在の進行プロジェクト: **PACING_V2.md(難易度調整AI作り直し・方向転換フォーク)**。
  どちらのチャットも作業開始時にまず PACING_V2.md(冒頭の「運用」「実装順とステータス」
  「★未決事項」)と DEVELOPMENT_LOG.md の先頭数エントリを読むこと。PACING_REDESIGN.md は
  前提知識(旧線の仕様)として参照可。矛盾したら PACING_V2.md が勝つ。
