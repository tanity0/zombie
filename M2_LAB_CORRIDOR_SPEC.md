...
- **【一時措置・社長指示v0.25.2256】`TUTORIAL_ALWAYS_SHOW = true`(`src/utils/tutorialArchive.ts`)の間は
  「1度だけ」を無効化し、出撃ごとに毎回出す**(社長が確認する手段が無いため)。記録は残るので資料室の
  「操作記録」は正しく埋まる。**完成したらこの定数を false に戻すだけで元の仕様に戻る**(社長が指示する)。
