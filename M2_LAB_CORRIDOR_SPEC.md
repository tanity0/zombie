...
- 縦配置: 帯の中を上下2段に分ける。位置は帯の**比率**で保持(帯を広げれば壁も追従)。
  上段 footY ∈ [-0.70L, -0.35L] / 下段 footY ∈ [0.55L, 0.95L](L=LAB_CORRIDOR_Y_LIMIT_PX)。
  L=200 の現在値: 上段の壁矩形 [-162,-70] / 下段 [88,190]。
- **通行保証(旧「廊下を塞がない」に代わる新しい不変条件)**: 中央 `[-70, 88]`(158px・L=200時)には壁を置かない。
...
| 中央の空きレーン | **`[-70, 88]`(158px・常時開通/帯に比例) | `src/world/labWalls.ts` `LAB_WALL_CLEAR_LANE` |
...
- 壁バーの幅が90pxで描画され、区画あたり最大8本(縦2段のラン)、**すべての壁が歩ける帯(±200)の内側**にあり、
  中央の空きレーン(帯に比例・L=200で `[-70,88]`)には壁が無い(=どのXでも通り抜けられる)。
