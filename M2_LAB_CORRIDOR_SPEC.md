...
- 縦配置: 帯の中を上下2段に分ける。上段 footY ∈ [-70,-35](矩形 [-92,-35]) / 下段 footY ∈ [55,95](矩形 [33,95])。
- **通行保証(旧「廊下を塞がない」に代わる新しい不変条件)**: 中央 `[-35, 33]`(68px)には壁を置かない。
...
| 壁生成レンジ | **歩ける帯 `|Y| ≤ 100` の中のみ**(v0.25.2228) | `src/world/labWalls.ts` |
| 中央の空きレーン | **`[-35, 33]`(68px・常時開通)** | `src/world/labWalls.ts` `LAB_WALL_CLEAR_LANE` |
...
- 壁バーの幅が90pxで描画され、区画あたり最大8本(縦2段のラン)、**すべての壁が歩ける帯(±100)の内側**にあり、
  中央の空きレーン `[-35,33]` には壁が無い(=どのXでも通り抜けられる)。
