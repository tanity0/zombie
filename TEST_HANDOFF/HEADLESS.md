...
## ★0-2. **画面は縦持ちで開く**(社長指摘2026-09-25「**このゲーム縦持ちだからね**」)

**このゲームは縦持ち。** 横持ち(1280×720)で測ると**実機では起きない問題を「見つけて」しまう**。

```js
const p = await b.newPage({ viewport: { width: 430, height: 932 }, deviceScaleFactor: 2, isMobile: true, hasTouch: true });
```

- **実例(v0.25.4640 → v0.25.4642 で差し戻し)**: ボスメーカーの部屋で「城ボスが出現直後に画面の外に居る」
  と判断して出現距離を 300→120px に変えた。**縦持ちで測り直したら -120/-200/-300/-400 のどれでも全部入る**
  (一番大きいステージ4の城ボスでも余裕がある)。**横持ちでしか起きない問題**だった。
- 「画面に入る/はみ出す」「寄り/引き」「上下の余白」に関わる判断は、**必ず縦持ちで測る**。
- 画素数を数える時は `deviceScaleFactor:2` を入れる(canvas は 860×1864 になる=実機と同じ2倍)。
